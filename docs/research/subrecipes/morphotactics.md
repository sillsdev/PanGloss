...
**The dossier's own status is "research-ready, implementation incomplete", and that is true of
`MorphotacticsSpec`. It is not true of morphotactics.** A real
`pangloss parse|batch --engine=foma` run compiles template legality every time, by a construction
this dossier does not describe.

**What ships.** Lexc continuation classes, not a typed spec:

- Per-template slot chains, one slot appearing exactly once per chain, an optional slot adding an
  epsilon skip — `rust/crates/pg-foma/src/emit.rs:1704` (`build_slot_chain`).
- Template grouping by exact shared `required_syn_fs` `FsId`, so templates with identical authored
  category lists share one root section — `rust/crates/pg-foma/src/emit.rs:2844`.
- Standalone derivation layers whose depth is the routed rule count rather than a constant, floored
  at `DERIV_DEPTH_MIN` — `rust/crates/pg-foma/src/emit.rs:239`, read in `build_deriv_chain`
  (`emit.rs:1541`) — plus outer, post-template layers so both attachment orders exist.
- A bounded compound loop and a computed depth cap — `emit.rs:1294`, `capability.rs:1532`, budget
  `emit.rs:249`.
- A subset-construction pruning automaton over the engine's own morphotactics, consulted immediately
  before each recursive step of the composite expansion —
  `rust/crates/pg-foma/src/morphotactics.rs:443` (`MorphotacticIndex`), `:563` (`next_state`), with
  the deliberately strict vacuity test at `:395`.

**How that differs from the dossier.**

| Dossier | Shipped |
|---|---|
| Enumerate complete legal template alternatives *first*, prune mutex and obligatory co-occurrence before materialization | Materialize a continuation graph, then restrict the composite recursion to engine-legal adjacencies with `MorphotacticIndex` |
| Legality expressed as an executable-recipe contract (`MorphotacticsSpec`) | Legality expressed as graph shape: a slot that cannot follow another has no arc |
| Rejects a Boolean product of independent optional slots as the wrong relation | Agrees in effect, by a different route: an earlier depth-N "bag" design *was* that product, and was replaced |
| No measured stage counters — every Task 6 field reads "Not measured — blocks implementation claim" | Measured: the bag design it replaced overgenerated by six orders of magnitude on one real word (2.5M candidates against the engine's 8, `emit.rs:17-23`); one reference grammar's 24 templates collapse to 9 groups (`emit.rs:53`) |

**Verdict.** The dossier is a specification of an alternative, not of unbuilt work. Its distinctive
content — complete-alternative enumeration ahead of materialization, and typed co-occurrence units —
is genuinely absent from the shipped path, and would be the thing to build if a grammar demanded it.
`MorphotacticsSpec` and the rest of `recipe_mechanism.rs` have no production caller
(`../recipe-machinery-audit.md`), so nothing on a real run constructs one today.

**Read alongside.** `../mainline-selection-audit.md` §B3; `../technique-index.md` §2.2-2.6, §2.14;
`../../fst-plan/morphotactic-composite-pruning.md` for the pruning measurements.
...
- [Linguistic construct harvest and citation ledger](../../fst-plan/linguistic-recipe-harvest.md), especially
...
- The bibliography entry for Tuggy (1991), Swift (1988), and Weber (1989) is preserved in the
  [harvest ledger](../../fst-plan/linguistic-recipe-harvest.md); a stable public scan was not found in the
...
[morphotactic-composite-pruning.md](../../fst-plan/morphotactic-composite-pruning.md)'s "vacuous rules in
