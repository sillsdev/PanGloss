...
> **Current FST policy (2026-08-25).** Capability correctness, compile execution limits, and
> readiness health are separate axes. `--allow-unproven` may lose valid parses and is for local
> testing only; publication rejects it. It never disables the configurable, finite execution
> limits. Retire `--remove-size-limits` and `--no-enforce-capability` without compatibility shims.
