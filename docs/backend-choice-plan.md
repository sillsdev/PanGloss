...
| `profile.rs` `CompileProfile` | Per-stage compile timings + final state/arc counts from the production pipeline. Profiles produced by this module carry the `Production` label; the raw measurements remain available in the profile output. | **No** (tuned path only) | Yes, inside `FomaProposer::new_with_budget`. |
