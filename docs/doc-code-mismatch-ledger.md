...
> `--allow-unproven` remains developer-build-only. It may lose valid parses and may write local
> developer evidence, but never production-publishes or certifies. The removed
> `--no-enforce-capability` and `--remove-size-limits` spellings are rejected in every build. They
> are rejection tombstones, not live controls. No production switch removes finite external limits;
