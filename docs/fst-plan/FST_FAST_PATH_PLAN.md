...
> **UPDATE (2026-07-07): Phase I of `FST_FULL_GRAMMAR_PLAN.md` is EXECUTED — I0 through I7 all
> DONE** (suite 144/144; commits `b9578909`..`15c5190d` on `fst-advisor`; I8 remains optional
> backlog, per its own spec, until a grammar needs Clitic/Process coverage). The general per-rule
> chain exists end-to-end: env-NFA + substitution/deletion/epenthesis/metathesis inverse compilers
> with the three-tier honesty report (Exact/Permissive/IdentitySkip + reasons), ONE shared product
> walker (the bare walk = a length-1 delegation), the boundary tape (byte-identical bare walk
> verified on both corpora), a two-axis beam cap (measured 1,000,000 default), and
> `ChainPhonologyProposer` wired into the composite — **OPT-IN, by the plan's own measured-decision
> rule: chain-on holds coverage exactly (Indonesian 121/121, 0 unsound) but regresses walk p50 ~37×,
> so v1 stays the default fast path and nothing is retired yet.** Junction-probing retirement's
> coverage half is answered (I4: the chain covers 46/46 meN- words with probing OFF); the perf half
> awaits future chain-walk optimization. KNOWN_GAPS resolution: the `PhonologyRuleCompiler` boundary
> gap and v1 scope limits — SUPERSEDED (the new compiler + boundary tape handle boundary-conditioned
> rules, deletion, epenthesis, metathesis, quantified envs; v1 stays in service purely as the
> measured-faster default, its gaps covered by the chain being wireable where they bite); the
> frontier beam cap — CLOSED (I6 + the calibration fix); §3b's chain walker — BUILT (I2). A third
> real grammar (Amharic, 417-segment syllabary) was added as a compiler target: 6 of 7 rules
> compile.
>
> Previous status (2026-07-03), for history: the only remaining item was Phase I in
> `FST_FULL_GRAMMAR_PLAN.md` — the lazy per-rule-chain generalization (the true-FST path), a full
> execution spec (milestones I0–I7 + optional I8, commit-gated), targeting correct-by-construction
> coverage of arbitrary regular HC grammars.
...
- ✅ **No frontier-beam cap on the NFA walk — CLOSED (Phase I).** The two-axis `BeamBudget`
  (default 1,000,000, empirically calibrated; latches on overflow, drops the word to
  "unparsed, counted", never throws or hangs) landed with the general chain — see the STATUS block
  above and `HYBRID_FST_FEASIBILITY.md` §8.1. Successor gap: overflow is *terminal* — see the
  "beam overflow is a coverage sink" entry below.
...
- **`EpsilonClosure`'s internal buffers are not pooled** (`result`/`seen`/`stack` are freshly
  ...
  through carefully).
- ✅ **Indonesian `mengamat-amati` — CLOSED (2026-07-03, Phase G1).** The suffix-peel inside the
  separator scan landed; Indonesian is 121/121 fully covered (see the STATUS block above and
  `docs/FST_FULL_GRAMMAR_PLAN.md` Phase G1).
...
- **Circumfixes are covered ONLY under the `forwardSynthesis` opt-in** (found by re-auditing the
  Phase 4 construct sweep, 2026-07-10): `MorphOp.CircumfixPrefix`/`CircumfixSuffix` are handled in
  `ForwardSynthesisProposer`'s covered-ops, but the bare FST has no circumfix arcs and no dedicated
  `IConstructProposer` exists — with forward synthesis off (the default), circumfix words fall to
  the engine. Not listed in `HYBRID_FST_FEASIBILITY.md` §8.5's residuals until now; fixed there as
  §10.2. Same bucket as clitics/process: no grammar in hand uses it, but any FLEx grammar could.
- **No verify-side work budget — `FstReplay.Confirm`'s termination is empirical, not bounded.**
  Pinning the root and rules collapses the *lexical* fan-out, but phonological un-application
  inside the restricted search is still a search; nothing caps it. On every grammar in hand
  restricted verify is a few ms, but a pathological candidate/grammar pair could hang the verifier,
  and the fast path has no latch-and-report guard there (unlike the walk's beam cap). Fix: a verify
  work budget with the same latch semantics as `BeamBudget`, plus a per-construct-class measurement
  battery for why restricted analysis stays bounded. See `HYBRID_FST_FEASIBILITY.md` §10.7.
- **Beam overflow is a coverage sink — latch-and-drop has no escalation path.** The Phase-I
  `BeamBudget` correctly can't hang, but an overflowed word is *lost to the fast path* (falls to
  engine/reported). Fine under the fallback architecture; incompatible with a 100%-fast-path goal.
  Fix direction: bounded escalation on overflow — iterative deepening of the budget and/or
  re-walking with proposers restricted per construct class. See `HYBRID_FST_FEASIBILITY.md` §10.6.
- **No completeness instrument — the "superset, never silent skip" contract is per-compiler
  discipline with no automated check.** A compiler that silently under-generates (emits a proper
  subset of the true inverse WITHOUT downgrading its tier) passes every soundness test and every
  tier report; only a coverage oracle can expose it, and the current oracle is engine *analysis* —
  the thing that hangs. Fix speced (`HYBRID_FST_FEASIBILITY.md` §10.1): a **forward-generation
  oracle** — enumerate root × rule-sequence combos to bounded depth, synthesize each surface form
  with the real engine (synthesis is tractable and deterministic), assert the fast path recovers
  that exact analysis. Should run as a standing generative battery per grammar, like the tier
  report.
- **Fixed caps that can silently cost coverage on an unseen grammar** (each individually pinned,
  collected here as a class): the chain's deletion floors count restoration *events* while an
  engine round restores multiple *sites* (known narrowing, pinned by test); the reduplication peel
  is bounded at ≤2 applications; the compound loop is bounded at 2 roots (lifting it requires the
  cycle defense documented in `HYBRID_FST_FEASIBILITY.md` §8.5). Fix direction: derive every cap
  from the grammar/engine's own bounds (engine rounds, actual reduplication rule structure,
  `MaxStemCount`) instead of constants — `HYBRID_FST_FEASIBILITY.md` §10.5.
- **MPR/environment/stem-name build-time gating graduates to a requirement under the
  100%-fast-path goal** (upgrade of the precision-gap entry above, not a new gap): with no engine
  fallback as a pressure valve, a grammar with heavy homography and open gating can multiply
  candidates until verify traffic dominates — a throughput cliff becomes a coverage cliff. See
  `HYBRID_FST_FEASIBILITY.md` §10.9.
