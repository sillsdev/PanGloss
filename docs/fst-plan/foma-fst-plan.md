...
bundle `pg_wasm_bg.wasm` = **1.58 MB** (budget < 10 MB). Two wasm32 RUNTIME crashes had to
...
- **MPR/POS rule-exception gating — DONE (2026-07-18, branch `p6-mpr-pos-flags`, not yet
  ...
  reproducing Amharic `prule1`'s `requiredPartsOfSpeech` shape 1:1) are in `pg-foma/src/gate.rs`'s
...
lands in main):** that branch adds a machine-conformance fixture for the pg-foma Latin-diacritic
