...
| **Proposer looseness** | See below | `pangloss fst-health <grammar> <words.txt>` (shallow); `rust/tools/pg.ps1 -Mode run -Example lab -- deadend_census <grammar>` (deep, attributed) | The deep tool exists but is a manual workflow (`.claude/skills/dead-end-census`), not wired into automatic health output — see Part 4. |
