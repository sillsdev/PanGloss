...
| 006 | [Discontinuous-morph environment anchoring (W3.3)](006-discontinuous-morph-env-anchor.md) | behavioural | fixed-in-rust | `Word.GetMorphs`/`MarkMorphs` (Word.cs) | `pg-rules/src/validity.rs`, `pg-rules/src/morph.rs::attribute_morphs` | `rust/conformance/allomorphy/discontinuous-env/`, `pg-parse/tests/discontinuous_env_gate.rs` |
...
| 016 | [`RewriteMode::Simultaneous` real semantics](016-simultaneous-rewrite-mode.md) | behavioural | fixed-in-rust | `SimultaneousPhonologicalPatternRule.Apply` | `pg-rules/src/rewrite.rs::sim_feature` | `rewrite/simultaneous-feeding` conformance fixtures |
