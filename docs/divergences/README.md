...
| 010 | [Root lookup needs unification, not char-def identity, on feature-bearing tables](010-anchor-rules-root-unification.md) | behavioural | fixed-in-rust | `CharacterDefinitionTable.Add`/`FeatureStruct.IsUnifiable` | `pg-grammar/src/chardef.rs` (`unif_closure`), `pg-parse/src/root_trie.rs` | `csharp_port_rewrite.rs::anchor_rules` |
...
| 031 | [Analysis cascade memoization](031-analysis-cascade-memo.md) | efficiency | superseded by 040 | `AnalysisStratumRule / AnalysisScope` | _(removed from Rust)_ | See entry: describes the pre-removal state |
| 032 | [Template-battery memoization](032-template-battery-memo.md) | efficiency | superseded by 040 | `AnalysisAffixTemplatesRule / AnalysisScope` | _(removed from Rust)_ | See entry: describes the pre-removal state |
...
| 040 | [Analysis memoization removed from HC-Rust](040-memoization-removed.md) | behavioural under a budget | removed-in-rust | `AnalysisScope / AnalysisStateKey` | `pg-memo` deleted; `pg-rules/src/analysis_state_key.rs` keeps the merge key | `-Mode test` + `-Mode conformance-test -Scope all`; `docs/research/memo-sena-bisect.md` and four companion measurement docs |
