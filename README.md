...
> **Developer-only diagnostic policy (current policy).** `--allow-unproven` is a developer-build/test
> control, not a production CLI capability; a production build must hide and reject it. It may omit
> valid parses, so its output is never certified or published. Capability enforcement, managed
> internal budgets, worker isolation, bounded I/O, and the non-disableable absolute ceiling remain
> active for every compile. `Error` health evidence remains production-unready and `Critical` denotes
> a correctness gap.
