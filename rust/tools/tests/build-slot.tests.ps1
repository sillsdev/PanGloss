...
Test-Case 'MaxConcurrent is honoured per invocation, not frozen by the first caller' {
    # The semaphore could not do this: its maximum was fixed by whichever process created it FIRST
    # and was unqueryable -- measured on this machine as three concurrent builds under a nominal
    # limit of 2. Here the slot count is just how many mutex names you wait on, so a caller asking
    # for 1 genuinely cannot take a second slot even though slot1 is free for 2-slot callers.
...
Test-Case 'Exit-BuildSlot tolerates null and a double release without throwing' {
    # It runs inside a finally, so it must never be the thing that turns a build failure into a
    # confusing PowerShell error on top of it.
...
# ---------------------------------------------------------------------------------------------
# Ledger: DIAGNOSTIC ONLY. It must never be consulted to decide whether a slot is free -- the
# mutexes are the exclusion and the kernel is the recovery.
# ---------------------------------------------------------------------------------------------
...
Test-Case 'commit charge is reported and is internally consistent' {
    # The number that was missing when a git fork failed on MEM_COMMIT while doctor showed 52GB of
    # available PHYSICAL memory. procgov --maxjobmem and event 2004 are both commit-denominated.
