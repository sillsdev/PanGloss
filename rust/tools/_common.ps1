...
function Import-PanGlossPlatformAdapter {
    ...
    $global:PanGlossPlatformAdapter = [PSCustomObject]@{
        Platform  = 'Linux'
        Overrides = @(
            'ConvertFrom-LinuxMountField', 'ConvertTo-LinuxCanonicalPath', 'Read-LinuxPlatformText',
            'Convert-LinuxKiBToBytes', 'Get-LinuxMemorySnapshot', 'Get-AvailableMemoryGB',
            'Get-TotalMemoryGB', 'Get-CommitChargeGB', 'Read-LinuxUnifiedMembership',
            'Get-LinuxCgroupMounts', 'Test-LinuxPathContains', 'Join-LinuxMappedCgroupPath',
            'Read-LinuxCgroupMemoryCap', 'Get-LinuxHostCgroupPreflight', 'Get-LinuxFreeSpaceGB',
            'Resolve-LinuxTargetDir', 'Use-LinuxSccache', 'Invoke-LinuxDirectProcess',
...
function Get-CommitChargeGB {
    ...
    try {
        ...
        if ($os -and $null -ne $os.TotalVirtualMemorySize -and $null -ne $os.FreeVirtualMemory) {
            $limitExact = ([double]$os.TotalVirtualMemorySize) * 1KB / 1GB
            $freeExact = ([double]$os.FreeVirtualMemory) * 1KB / 1GB
            $committedExact = $limitExact - $freeExact
            $limit = [math]::Round($limitExact, 1)
            $free = [math]::Round($freeExact, 1)
            return [PSCustomObject]@{
                LimitGB     = $limit
                FreeGB      = $free
                FreeGBExact = $freeExact
                CommittedGB = [math]::Round($committedExact, 1)
                PercentUsed = if ($limitExact -gt 0) { [int][math]::Round(($committedExact / $limitExact) * 100) } else { $null }
            }
        }
    } catch {}
    return $null
...
function Write-Preflight {
    ...
    foreach ($pool in @('build', 'run')) {
        ...
        # Snapshot lazily, since it is costly; Get-ProcessSnapshot is Windows CIM, so Linux lists holders without a staleness verdict.
