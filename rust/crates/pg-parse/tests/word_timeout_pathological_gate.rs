...
//!
//! `pg-rules/src/stratum.rs`'s `step_budget_timeout_tests` already prove the deadline mechanism
...
//! `pg-rules/tests/stratum_gate.rs`'s `merge_equivalent_analyses_folds_homophonous_suffixes...`
...
/// run would make), and — since the O1b fix (`pg-rules/src/stratum.rs`'s `StepBudget::over_budget`
...
/// regression guard for that is `pg-rules/src/stratum.rs`'s
...
fn fifty_ms_word_timeout_fires_promptly_on_a_genuinely_slow_parse() {
    ...
    // Bound history: was 5_000; pg-fst's Rc/COW-register + min-hops-pruning traversal change cut
