...
use hc_featstruct::bitvec::flat_unifiable;
use hc_grammar::chardef::{CharDefId, CharDefKind};
use hc_grammar::model::{Grammar, NaturalClassKind, TableId};
...
fn load(name: &str) -> Option<(Grammar, String)> {
    ...
    let g = hc_grammar::load(&xml).unwrap_or_else(|e| panic!("failed to load {name}: {e}"));
    Some((g, xml))
}
