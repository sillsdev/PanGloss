...
fn simulfix_synthesis_voices_target_segment() {
    ...
    let vp = g
        .phon_features
        .symbol_index(hc_grammar::featsys::FlatIndex(voi as u32), "sym_vp")
        .unwrap();
    ...
    // rendering/matching as its PRE-modification character forever). `hc_shape::Shape::node_cd_set`
    ...
    let s = &out[0].shape;
    assert_eq!(char_defs(s), vec![cd(&g, "char_a"), hc_shape::NO_CHAR_DEF]);
    assert_eq!(
        s.node_lanes(2)[voi],
        voiced_bits,
...
fn compound_rule() -> MorphRuleDef {
    MorphRuleDef::Compounding(CompoundingRuleDef {
        ...
        blockable: false,
        max_apps: 1,
        head_required_syn_fs: hc_featstruct::FsId(0),
        non_head_required_syn_fs: hc_featstruct::FsId(0),
        out_syn_fs: hc_featstruct::FsId(0),
...
fn sena_affix_and_compounding_rules_compile_and_run_without_panic() {
    ...
    let g = hc_grammar::load(&xml).expect("Sena grammar loads");
    ...
    for rule in &g.mrules {
        match rule {
            MorphRuleDef::AffixProcess(def) => {
                affix_rules += 1;
                let bridge = hc_rules::bridge::PatternBridge::new(&g).with_table(TableId(0));
