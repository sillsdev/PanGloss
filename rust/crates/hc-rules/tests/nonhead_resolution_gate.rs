...
//! `hc_rules::morph::resolve_non_head_roots` (called from `ana_compound_subrule`, threaded via
//! `NonHeadRootFilter`, the same crate-boundary shape `hc-parse::Morpher::set_root_allomorph` uses
...
//! once cache" items), independent of this fix. Calling `hc_rules::morph::{analyze_with_root_filter,
...
use hc_rules::morph::{analyze_with_root_filter, synthesize};
use hc_rules::stratum::NonHeadRootFilter;
...
fn load_sena() -> Option<Grammar> {
    ...
    Some(hc_grammar::load(&xml).unwrap_or_else(|e| panic!("Sena grammar failed to load: {e}")))
}

/// A feature-bearing shape from `text` (mirrors `morph_gate.rs`'s `shape_with_lanes`; table 0 is
/// the one Sena's morphological rules resolve char-defs against, matching `hc_rules::morph`'s
/// `TABLE` constant).
...
fn nonhead_resolution_replaces_shape_and_syntactic_fs() {
    ...
    // Stand-in for `hc-parse::RootAllomorphIndex::search`: a real (surface-keyed) lexicon lookup
    // over just these two entries, exactly the shape `hc-parse::Morpher`'s production filter has
    // (`RootAllomorphFilter`'s doc in `hc_rules::stratum`).
...
/// see `hc_grammar::model`). Test-only: production code never needs to clone a loaded rule.
fn clone_def(def: &CompoundingRuleDef) -> CompoundingRuleDef {
    CompoundingRuleDef {
        ...
        subrules: def
            .subrules
            .iter()
            .map(|sr| hc_grammar::model::CompoundingSubruleDef {
                vars: sr.vars.clone(),
...
fn synthesis_records_non_head_root_morph_in_the_final_signature() {
    ...
    // `hc-parse::Morpher::synthesis_pipeline` would hand to `synth_compound`): head = "ico" as a
    // resolved root word (mirroring `hc-parse::Morpher::set_root_allomorph` on the head, out of
