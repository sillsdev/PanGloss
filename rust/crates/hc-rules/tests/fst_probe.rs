...
fn match_span_is_physical_in_both_directions() {
    ...
    let lhs = Pattern { nodes: vec![PatternNode::CharDef(common::char_def(&g, "char_t"))] };
    ...
    // ---- LtoR ----
    let fst = compiled.input.compile_with_direction(Direction::LeftToRight);
    ...
    assert_eq!(node_of[1], 2, "seg 1 maps to shape node 2 (LA=0, a=1, t=2, a=3, RA=4)");
    ...
    let fst_r = compiled_nd.input.compile_with_direction(Direction::RightToLeft);
    ...
    let spans_r: Vec<(i32, i32)> = res_r
        ...
        .collect();
    assert_eq!(spans_r, vec![(1, 2)], "RtoL: get_offsets un-swaps back to the physical span [1,2)");
}
...
fn flat_unifiable_is_not_usedefaults() {
    ...
    assert!(hc_featstruct::flat_unifiable(&[0b10, 0b01], &[u64::MAX, 0b01]));
    ...
    assert!(hc_featstruct::flat_unifiable(&[u64::MAX, 0b01], &[0b01, 0b01]));
}
