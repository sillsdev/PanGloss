//! Alpha-variable agreement gate: the FST over-approximates variable-governed lanes, so hc-rules
...
fn seg(g: &Grammar, w: &str) -> Shape {
    hc_rules::shape_feat::segment_with_features(g, table(g), w).unwrap()
}
...
fn nasal_assimilation_applies_the_bound_place() {
    ...
    let out = hc_rules::rewrite::synthesize(&g, &r, &seg(&g, "ng"));
