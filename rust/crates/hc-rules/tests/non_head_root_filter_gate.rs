...
//!
//! `hc-rules` cannot depend on `hc-parse` (crate-boundary: `hc-parse` depends on `hc-rules`, not the
//! reverse — plan §5.1 crate map), so [`hc_rules::stratum::NonHeadRootFilter`] is injected. These
//! tests stand in for `hc-parse`'s `RootAllomorphIndex::search` with a hand-written closure over a
...
use hc_featstruct::{FeatId, FeatureStructBuilder, FeatureValue, FsId, SymbolBits};
...
/// back through `g.morphemes[..].stratum` (mirroring `hc-parse::Morpher::set_root_allomorph`), so
...
/// Tier-2 #7's resolution (`hc-rules::morph::resolve_non_head_roots`) has a real `RootAllomorphDef`
...
fn syn_fs(bit: u32) -> hc_featstruct::FeatureStruct {
...
// backward compatibility with every pre-existing (lexicon-free) hc-rules test.
// =================================================================================================
