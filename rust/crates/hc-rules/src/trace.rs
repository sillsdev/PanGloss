...
//! here — `hc-rules`/`hc-parse`'s own functions gain `trace` parameters in later chunks. This module
...
use hc_grammar::model::{MRuleId, PRuleId, StratumId, TemplateId};
...
/// (`hc_rules::cascade::Cascade::linear`/`combination`) take their per-rule closure as `A: Fn(usize,
...
impl TreeTraceSink {
    ...
    /// (see this module's tests and `hc-cli`'s renderer, chunk 7).
