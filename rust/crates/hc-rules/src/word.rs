...
use hc_featstruct::{FeatId, FeatureStruct};
use hc_grammar::model::{AllomorphId, LexEntryId, MRuleId, MorphemeId, MprSet, StratumId};
use hc_shape::Shape;
...
pub struct MorphRecord {
    ...
    /// [`MorphStatus`] and `hc_rules::morph::attribute_morphs`'s doc comment for the C# mechanism
...
/// records own output nodes (`hc_rules::morph`'s `owning_morph` skips the rest); the other three
...
pub enum MorphStatus {
    ...
    /// `hc_rules::morph::FLOATING_ORDER` sentinel while in this state.
    Floating,
...
/// See `hc-parse/src/guess.rs`'s module doc (P11 chunk 4-5) for how this is fabricated, and
/// `hc-rules/src/validity.rs::allomorphs_valid_impl`'s sentinel branch (P11 chunk 3) for the
...
pub struct GuessedRoot {
    ...
    /// The rendered guess string (`hc-parse/src/guess.rs::render_match`'s output). Doubles as C#'s
...
pub struct Word {
    ...
    /// `synth_realizational` (`hc_rules::morph`), or on a generation seed built by
    /// `hc_parse::morpher`'s `generate_words` with a caller-supplied non-empty value (C#
    ...
    /// analysis, only from [`hc_parse`]'s `generate_words` seeding a bare `LexEntry` non-head
    /// directly into the unapplication trail (W7); the synthesis confirmation gate
    /// (`hc_rules::stratum::guided_synth`) matches a `None` slot against **any** `CompoundingRule`,
    ...
    /// deferred from M4b). Its sole consumer is [`hc_memo::AnalysisStateKey`], which needs an
...
/// (Word.cs:508-546). The cascades (`hc_rules::cascade`) and the stratum orchestrator dedup on this
...
impl Word {
    ...
    /// null (only reachable from [`hc_parse`]'s `generate_words`, never from analysis). If `mrule` is
    ...
    /// `hc_rules::morph::synth_realizational`, not the ordinary trail), the slot is pushed and the
    /// index advanced.
    ...
    /// onto `query` — a different word that reached the same [`hc_memo::AnalysisStateKey`] as `N` via
    ...
    /// `non_heads` (P4, 2026-07-09 — see `hc-rules/src/morph.rs`'s `synth_compound_subrule` doc for
    /// why `non_heads` is deliberately never popped). With two or more non-heads on the trail
    /// (reachable today via `hc_parse::Morpher::generate_words`'s direct API, which pushes one
    /// non-head per `GenMorpheme::NonHead` with no `max_stem_count` gate — that gate is analysis-
    /// side only, `hc-rules/src/stratum.rs`'s `apply_one_mrule`, and generation never goes through
            ...
            Some(o) if o.len() >= 2 => {
                ...
                for original in o {
                    ...
                    if self.real_fs != src.real_fs {
                        let diff = hc_featstruct::subtract(&self.real_fs, &src.real_fs);
                        if let Some(new_fs) = hc_featstruct::unify(&alt.real_fs, &diff) {
                            alt.real_fs = new_fs;
                        }
                    }
