...
//! shape machinery `hc_rules::rewrite` uses (reused, not duplicated — both ports independently need
...
//! [`hc_grammar::model::MetathesisRuleDef`] carries ONE compiled pattern (no separate LHS/RHS split,
...
//! `hc_rules::bridge::PatternBridge`) to represent a switch. This port does that lowering **post-hoc**
//! instead ([`compile_switch_pattern`]): compile the plain pattern via `PatternBridge` as usual, then
//! wrap the two switch positions' already-compiled nodes in a named `hc_fst::CompileNode::Group` and
//! recover their matched spans via `Fst::get_offsets` after a match — exactly the technique
//! `hc_rules::rewrite::compile_env_impl` already uses to recover alpha-variable positions (Tier-2
//! #12), and the same primitive `hc_rules::morph::compile_parts` uses for affix-part captures. This
...
//! (mirroring `hc_rules::rewrite::syn_feature`'s identical, already-documented choice for the same
//! "a feature-changed node must stop being treated as a concrete, single-char-def-identity node"
//! reason).
//!
//! ## MPR/POS immunity
//! No subrule-level gating exists at all (see [`hc_grammar::model::MetathesisRuleDef`]'s doc) — every
...
/// `hc_rules::rewrite::compile_env_impl`'s identical `CompileNode::Group` post-processing for
/// alpha-variable position recovery.
///
/// **Direction handling** (verified against `hc-fst`'s own test suite,
/// `crates/hc-fst/tests/fst.rs`'s "Class 5: RightToLeft traversal" guards — this module is the
/// first `hc-rules` caller to compile a *multi-node* pattern with `Direction::RightToLeft`, since
/// `hc_rules::rewrite`'s own LHS/RHS patterns never carry more than the rule-level direction their
...
/// `hc_rules::rewrite::all_spans`'s convention, extended with the two named group offsets
/// `hc_rules::rewrite`'s single-pattern rules never need (metathesis is the first rule kind whose
...
/// anchor is a call-site flag, never baked into the compiled `Fst` itself — `hc_rules::bridge`'s
/// module doc).
...
/// unconstrained" (`UNCONSTRAINED = u64::MAX`, `hc_rules::bridge`'s module doc), so a plain bitwise OR
...
///
/// Also resets both nodes' `char_def` to `NO_CHAR_DEF` (`hc_shape::NO_CHAR_DEF`), mirroring
/// `hc_rules::rewrite::syn_feature`'s identical, already-documented choice: after a lane-widening
...
fn ana_union(ms: &mut MutShape, left: &[usize], right: &[usize]) {
    for (&a, &b) in left.iter().zip(right) {
        ...
        ms.nodes[a].char_def = hc_shape::NO_CHAR_DEF;
        ms.nodes[b].char_def = hc_shape::NO_CHAR_DEF;
    }
}

// =================================================================================================
// Public API (mirrors `hc_rules::rewrite::synthesize`/`analyze`'s signatures/return convention).
// =================================================================================================
...
/// `hc_rules::rewrite::synthesize_with_mpr_traced`'s convention).
...
/// `hc_rules::rewrite::synthesize_with_mpr_cached_traced`.
...
// `hc_rules::rewrite::analyze_cached_traced` isn't: `crate::stratum::StratumAnalyzer::analyze`
...
/// `pub(crate)` visibility rules out the "export it `pub`" dodge `hc_rules::rewrite::
