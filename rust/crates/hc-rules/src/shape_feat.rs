...
//!
//! `hc_grammar::segment` returns width-0 shapes (segmentation gate only). The phonological-rule
...
use hc_grammar::segment::InvalidShape;
use hc_shape::{Shape, ShapeBuilder};

/// Segment `word` against `table` and attach per-node phonological feature lanes (`W` lanes each,
/// `W = grammar.phon_features.len()`, always >= 1 — see `hc_grammar::featsys` module docs on the
...
pub fn segment_with_features(
    ...
    let bare = hc_grammar::segment::segment(table, word)?;
    ...
    for (_, kind, char_def, _flags) in bare.interior() {
        ...
        match kind {
            hc_shape::NodeKind::Segment => {
                b.push_segment_with_lanes(char_def, &lanes);
            }
            hc_shape::NodeKind::Boundary => {
...
/// (`hc-grammar/src/chardef.rs`), so the `raw.len() == w` fast path is the common case for both
...
/// for robustness — mirrors `hc-rules/src/morph.rs`'s `fit`).
