...
//! Morpher is a later milestone): given a [`hc_grammar::model::RewriteRuleDef`] and a
...
//! `hc_shape`/`hc_fst` contracts do **not** encode as lanes:
...
//!
//! The FST matcher (`hc_fst`) also cannot bind alpha variables or apply `UseDefaults` feature
...
// `hc_shape::Shape` does not carry).
// =================================================================================================

// `pub(crate)` (not `pub`, still crate-private): `hc_rules::metathesis` (a sibling module, not a
...
// precisely what the metathesis synthesis reorder (`hc_rules::metathesis::synthesis_reorder`) needs,
...
impl MutShape {
    ...
    /// boundaries — see the identical fix/rationale on `hc_rules::morph::segs_of`. Within this
...
/// changes scan order/preference, not the matched string). `hc_fst`'s own frozen convention is the
...
/// `hc_fst` primitive for the same "recover a sub-match's real position" need.
///
/// Empirically probed (P6 investigation): a group's START offset is always the row's true
/// single-segment position, even when `hc_fst::traverse::Transduce::advance`'s "skip the next
...
/// pattern it matched: `hc_fst::traverse::Transduce::advance`'s "skip the next Optional
...
pub(crate) struct EnvFst {
    ...
    ///
    /// The Rust side has this problem only because `hc-fst`'s frozen FSA path cannot carry variables
    ...
    /// hc-fst's existing frozen `CompileNode::Group`/`Fst::get_offsets` capture primitive (already
    /// used identically by `hc_rules::morph::compile_parts`/`part_ranges` for affix-part captures) to
    ...
    /// independent of how many segments any quantifier in the pattern actually consumed. No hc-fst
...
fn compile_env_impl(
    ...
    // (see `EnvFst::group_names`'s doc). Mirrors `hc_rules::morph::compile_parts`'s identical
    // `CompileNode::Group` wrapping for affix-part captures — same frozen hc-fst primitive, no
    // hc-fst change. Nodes with no alpha-variable occurrence are left unwrapped (no capture needed).
...
///
/// `hc_fst`'s frozen contract has no analog of "unset vs. explicitly full-mask" (this module's own
/// doc already flags this as a gap `hc_fst` "cannot apply"), so the FST's own match is
...
/// (`hc_grammar::featsys::PhonFeatureSystem::default_bits`), the candidate is only really valid if
...
/// [`hc_grammar::model::Grammar::mpr_group_ok`] (W3.1: MPR-group-aware; this was a flat overlap
...
/// after the fix (see `hc-parse/tests/csharp_port_rewrite.rs::boundary_rules_required_pos_on_subrule
...
/// [`hc_grammar::model::RewriteSubruleDef::required_pos`] — already loaded from
/// `requiredPartsOfSpeech` as a POS symbol bitset (`hc-grammar/src/load.rs::parse_pos_bits`), the
...
/// (`hc-featstruct/src/ops.rs:106`) treats a feature present on only one side as never blocking. When
...
/// `Word.syn_fs` gating (`hc_rules::stratum::synthesize_stratum`'s trailing prule application) call
...
fn subrule_gate_reason(
    ...
    if !hc_grammar::model::mpr_required_ok(&g.mpr_groups, sr.required_mpr, mpr) {
        return Some(FailureReason::RequiredMprFeatures);
    }
    if !hc_grammar::model::mpr_excluded_ok(&g.mpr_groups, sr.excluded_mpr, mpr) {
    ...
    loop {
        ...
        for (s, e) in all_spans(target, &segs) {
            ...
            for (k, &node) in target_nodes.iter().enumerate() {
                ...
                // `hc_shape::Shape::node_cd_set` (plan §13.1 Tier-1 #3), added to fix a *different*
                ...
                // (`hc-parse/src/surface.rs`) would then only ever consider archiphoneme char29's
...
/// crates/hc-parse/tests/csharp_port_rewrite.rs::multiple_segment_rules_deletion_composition_
...
/// real target positions: `hc_fst::traverse::Transduce::advance`'s "skip the next Optional
...
fn ana_feature(
    ...
    // `hc-rules/src/rewrite.rs`'s `group_probe_diag` module): `hc_fst::traverse::Transduce::
    ...
    // `group_probe_diag::group_offsets_survive_interposed_optional_ltr` (a synthetic `hc_fst`-level
    ...
    loop {
        ...
        for row_starts in candidates {
            ...
            for (k, &node) in target_nodes.iter().enumerate() {
                ...
                ms.nodes[node].char_def = hc_shape::NO_CHAR_DEF;
            }
            applied = true;
            acted = true;
            break;
...
/// `hc_grammar::model::Dir` → `hc_fst::Direction`. `pub(crate)` so `hc_rules::metathesis` (whose
/// `MetathesisRuleDef.dir` is the same model `Dir`) can reuse it instead of duplicating the match.
pub(crate) fn dir_from_model(d: hc_grammar::model::Dir) -> Direction {
    match d {
        hc_grammar::model::Dir::LeftToRight => Direction::LeftToRight,
        hc_grammar::model::Dir::RightToLeft => Direction::RightToLeft,
    }
}
...
/// pre-fix over-approximation `hc_rules::morph`'s `InsertSimpleContext` sites had before this
    ...
    let (char_def, kind) = match node {
        PatternNode::CharDef(cd) => {
            let kind = match table.get(*cd).kind() {
                hc_grammar::chardef::CharDefKind::Segment => NodeKind::Segment,
                hc_grammar::chardef::CharDefKind::Boundary => NodeKind::Boundary,
            };
            (cd.0, kind)
        }
