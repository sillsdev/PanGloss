...
/// reverse — plan §5.1 crate map). The signature matches `hc-parse`'s
/// `RootAllomorphIndex::search`'s exact return shape (`(AllomorphId, LexEntryId)` pairs), so
/// `hc-parse` can hand its own method straight in as the closure body.
///
...
/// per-subrule dedup (cs:99-117) are all done on the `hc-rules` side instead (`morph::
...
/// `hc_rules::shape_feat::segment_with_features` to re-segment the matched allomorph's text) — no
...
/// The per-parse memo carrier this module threads through the analysis cascade. `hc-parse` owns one
/// per `parse_word` call (see `hc_memo::AnalysisScope`) and hands it in via [`analyze_stratum_scoped`].
...
/// hot loop is `hc-fst::traverse`, deliberately NOT touched here), so `Instant::now()`'s real cost
...
/// the whole descent — and, because `hc-parse` reuses one scope for every stratum + input word of a
...
/// — M5c). `hc-parse::Morpher` calls this (wiring in its `RootAllomorphIndex::search`); everything
/// else — `hc-rules`'s own lexicon-free tests included — keeps calling [`analyze_stratum_scoped`] /
...
/// production call site (`hc-parse::Morpher::parse_word`): `cache` is mandatory here — a `RuleCache`
...
impl<'g, 's, 'f, 'r, 'c, 'b> StratumAnalyzer<'g, 's, 'f, 'r, 'c, 'b> {
    ...
    fn template_unapply_slots(
        &self,
        tmpl: &hc_grammar::model::AffixTemplateDef,
        in_word: &Word,
        index: isize,
...
///   `id` names a `CompoundingRule` — reachable when [`hc_parse`]'s `generate_words` (W7) seeds a
...
/// a real handle once, at the top of the per-word synthesis pipeline (`hc-parse::Morpher`'s
...
fn synth_apply_mrules(
    g: &Grammar,
    stratum: StratumId,
    sd: &hc_grammar::model::StratumDef,
    input: &Word,
    cap: usize,
    steps: &Cell<usize>,
    cache: &RuleCache,
