...
//!
//! Translates an authored [`hc_grammar::model::Pattern`] (its [`PatternNode`] tree) into
//! [`hc_fst::CompileInput`] and compiles it to a frozen [`hc_fst::Fst`]. This is the seam the
//! module doc of `hc-fst`'s `compile.rs` describes: hc-fst deliberately does *not* depend on
//! hc-grammar; **this** module owns the grammar-aware resolution (natural classes, char-def
...
//!   one with 14 variable features). hc-fst's frozen FSA path carries **no** variable bindings (its
//!   determinism predicate is literally `!hasVariables`) — a **flagged frozen-contract gap**. We do
//!   not edit hc-fst; instead the bridge lowers every variable-governed feature lane to
...
//!   `hc_grammar::chardef::CharDef::feature_lanes` doc, `2f238cee`); a boundary constraint here
//!   matches only boundary nodes, exactly like any other pinned lane.
//! - [`PatternNode::Quantifier`]: the hc-fst `{min,max}` quantifier over the compiled children.
//! - [`PatternNode::Segments`]: a sequence of per-node constraints taken from the pre-segmented
//!   shape's interior char-defs.
//! - [`PatternNode::Anchor`]: hc-fst has no anchor *node* — anchoring is the `start_anchor`/
//!   `end_anchor` flags on the traversal (see `hc-fst` compile.rs docs). A left/right anchor node
//!   therefore lifts to a flag on the returned [`CompiledPattern`], not a `CompileNode`.

use hc_fst::{CompileInput, CompileNode, Fst};
...
/// An unconstrained lane (all symbols allowed), matching `hc_fst::lanes::UNCONSTRAINED` and
...
/// The agreement check that consumes these lives in `hc_rules::rewrite` (the frozen FST cannot bind
...
/// A construct in an authored pattern that the frozen hc-fst FSA path cannot express (flag, don't
...
/// [`PatternNode::Anchor`] nodes (which hc-fst expresses as traversal flags, not nodes).
#[derive(Clone, Debug)]
pub struct CompiledPattern {
    ...
    /// `hc_rules::rewrite` agreement check binds/verifies these against actual node lanes.
    pub node_vars: Vec<Vec<VarOccur>>,
}

impl CompiledPattern {
    /// Compile to a frozen forward FST (traversal direction defaults to `LeftToRight`). The rule
    /// drivers pass an explicit direction via [`hc_fst::CompileInput::compile_with_direction`].
...
pub struct PatternBridge<'g> {
    ...
    /// (`hc_rules::morph::compile_parts` / `build_analysis_lhs`) opt in, and only they feed these
...
impl<'g> PatternBridge<'g> {
    ...
    /// mechanism that motivated this milestone's fix on the *output* side (`hc_rules::morph`'s
    /// `InsertSimpleContext`, `hc_parse::surface::matching_str_reps`). Sena's own grammar exercises
    ...
    /// milestone**: `hc_fst::Segment` (the frozen FST's per-position match unit) carries only
    /// phonological lanes, no char-def/`StrRep` dimension, so discriminating by real membership
    /// here needs either a `hc-fst` representation change (a frozen contract this port does not
