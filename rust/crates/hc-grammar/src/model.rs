...
//! the input to every downstream crate (hc-fst pattern compile, hc-rules, hc-parse).
//!
...
impl MorphemeId {
    ...
    /// Content (id/gloss/join string) is delegated to `hc_rules::word::GuessedRoot::text`.
...
impl AllomorphId {
    ...
    /// (`hc_rules::word::GuessedRoot::pattern_allo`) — see that module's doc for the full list of
...
impl SynFeatureSystem {
    ...
    pub fn feature_by_xml_id(&self, xml_id: &str) -> Option<hc_featstruct::FeatId> {
        self.features
            .iter()
            .position(|f| f.xml_id == xml_id)
            .map(|i| hc_featstruct::FeatId(i as u16))
    }

    /// Resolve a symbol of `feat` by XML id → its bit index.
    pub fn symbol_index(&self, feat: hc_featstruct::FeatId, symbol_xml_id: &str) -> Option<u32> {
    ...
    /// The full-domain bitmask for `feat` (`hc_featstruct::full_mask(symbol count)`), i.e. every
    /// bit `hc_featstruct::ops::add` needs to detect a symbolic value that now allows every
...
// --- Patterns (authored AST; hc-fst compiles these at M2) -------------------------------------
...
pub struct RewriteSubruleDef {
    ...
    /// time (`hc_grammar::load::load_rewrite_subrule`) — a static fact about the rule's own
...
/// captured spans of one match pattern — see `hc_rules::metathesis` for the engine, and
...
pub struct MorphemeInfo {
    ...
    /// `Morpheme.MorphemeCoOccurrenceRules`). Evaluated by `hc-rules::validity` alongside the
...
impl MorphRuleDef {
    ...
    /// lookup here is what lets `hc-rules::cache`/`hc-rules::validity` treat an `AllomorphOwner::
...
/// `OutMprFeatures` application on synthesis either — see `hc_rules::morph`'s realizational doc for
...
pub struct AffixProcessRuleDef {
    ...
    pub obligatory_features: Vec<hc_featstruct::FeatId>,
    ...
    /// strata/templates are loaded (`hc_grammar::load`'s post-pass), not re-derived per word.
...
/// wrappers, the actual call sites in `hc-rules`.
...
pub struct Grammar {
    ...
    /// interpretable against it, and downstream crates (hc-fst pattern compile, hc-rules) need
    ...
    pub fs_interner: hc_featstruct::Interner<hc_featstruct::FeatureStruct>,
