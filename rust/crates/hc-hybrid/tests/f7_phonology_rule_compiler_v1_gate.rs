...
fn analyze_composed(
    ...
    pinv: hc_hybrid::inverse::InversePhonology,
    word: &str,
) -> Vec<hc_hybrid::walk::WordAnalysis> {
    let morpher = Morpher::new(g, usize::MAX);
    let underlying_trie = Trie::build_ex(g, surface, &morpher, 1_000_000, 2, false, false);
    walk::analyze_chain(
        g,
        &underlying_trie,
        std::slice::from_ref(&pinv),
        word,
...
fn bare_underlying_walk_is_empty(
    g: &hc_grammar::model::Grammar,
    surface: &SurfacePhonology,
    word: &str,
) -> bool {
    let morpher = Morpher::new(g, usize::MAX);
    let underlying_trie = Trie::build_ex(g, surface, &morpher, 1_000_000, 2, false, false);
    walk::analyze_word(g, &underlying_trie, word, walk::DEFAULT_MAX_BEAM_WORK)
        .analyses
        .is_empty()
}
...
fn composed_sigs(
    g: &hc_grammar::model::Grammar,
    analyses: &[hc_hybrid::walk::WordAnalysis],
...
fn compile_auto_recovers_boundary_deletion() {
    ...
    assert_eq!(
        ...
        "this rule is entirely within the v1 supported shape"
    );

    let surface = SurfacePhonology::new(&g);
    assert!(
        bare_underlying_walk_is_empty(&g, &surface, "sagd"),
        "baseline: the underlying-only walk alone must miss 'sagd'"
    );

    let composed = analyze_composed(&g, &surface, result.pinv, "sagd");
    assert!(
        composed.iter().any(|c| c.morphemes.len() == 2),
        "auto-compiled Pinv must recover the deletion form: {composed:?}"
    );
    assert!(
        composed_sigs(&g, &composed).is_subset(&engine_sigs(&g, "sagd")),
        "soundness: composed candidates must be a subset of the engine's"
    );

    let result2 = compiler_v1::compile(&g);
    assert!(
        analyze_composed(&g, &surface, result2.pinv, "saga").is_empty(),
        "a non-word must yield nothing"
    );
}
...
fn compile_auto_recovers_left_context_deletion() {
    ...
    assert_eq!(
        result.unsupported_rule_count, 0,
        "left-context-only deletion is within the supported shape"
    );
    ...
    assert!(
        composed.iter().any(|c| c.morphemes.len() == 2),
        "auto-compiled Pinv must recover the left-context deletion form: {composed:?}"
    );
    assert!(
        composed_sigs(&g, &composed).is_subset(&engine_sigs(&g, "sagd")),
        "soundness: composed candidates must be a subset of the engine's"
...
fn compile_auto_recovers_unconditioned_substitution() {
    ...
    let surface = SurfacePhonology::new(&g);
    assert!(
        bare_underlying_walk_is_empty(&g, &surface, "dad"),
        "baseline: the underlying-only walk alone must miss 'dad'"
    );

    let composed = analyze_composed(&g, &surface, result.pinv, "dad");
    assert!(
        !composed.is_empty(),
        "auto-compiled Pinv must recover the substituted form: {composed:?}"
    );
    assert!(
        composed_sigs(&g, &composed).is_subset(&engine_sigs(&g, "dad")),
        "soundness: composed candidates must be a subset of the engine's"
    );

    let result2 = compiler_v1::compile(&g);
    assert!(
        analyze_composed(&g, &surface, result2.pinv, "zzz").is_empty(),
        "a non-word must yield nothing"
    );
}
...
fn compile_skips_unconditioned_deletion_as_unsupported() {
    ...
    let result = compiler_v1::compile(&g);
    assert_eq!(
        result.unsupported_rule_count, 1,
        "unconditioned deletion must be marked unsupported, not compiled"
    );
}
...
fn compile_auto_recovers_left_context_substitution() {
    ...
    let surface = SurfacePhonology::new(&g);
    assert!(
        bare_underlying_walk_is_empty(&g, &surface, "kagd"),
        "baseline: the underlying-only walk alone must miss 'kagd'"
    );

    let composed = analyze_composed(&g, &surface, result.pinv, "kagd");
    assert!(
        !composed.is_empty(),
        "auto-compiled Pinv must recover the left-conditioned substitution: {composed:?}"
    );
    assert!(
        composed_sigs(&g, &composed).is_subset(&engine_sigs(&g, "kagd")),
        "soundness: composed candidates must be a subset of the engine's"
    );

    let result2 = compiler_v1::compile(&g);
    assert!(
        analyze_composed(&g, &surface, result2.pinv, "kagz").is_empty(),
        "a non-word must yield nothing"
    );
}
...
fn lockstep_phonology_proposer_covers_deletion_wired_through_composite() {
    ...
    assert!(
        !got.is_empty(),
        "composite (via the lockstep proposer) must recover 'sagd'"
    );
    ...
    assert!(
        lockstep.has_arcs(),
        "the merged Pinv must have real non-identity arcs for this grammar"
    );
...
fn quirk1_lockstep_proposer_misses_left_environment_gated_rule_from_state_zero() {
    ...
    assert!(
        !composed.is_empty(),
        "the compiled Pinv genuinely has a usable non-identity arc (one hop past state 0)"
    );
    ...
    assert!(
        !lockstep.has_arcs(),
        "quirk 1: a left-environment-gated rule's arc is invisible to a state-0-only scan"
    );
    assert!(
        lockstep.analyze_word(&g, "kagd").is_empty(),
        "quirk 1's downstream effect: the wrapper silently contributes nothing for this word"
    );
}
