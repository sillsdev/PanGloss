...
fn chain_recovers_word_internal_deletion_with_env_gating() {
    ...
    assert!(
        !morpher.parse_word("kaatai").analyses.is_empty(),
        "precondition: the engine parses 'kaatai' under the deletion rule"
    );
    ...
    assert!(
        walk::analyze_word(&g, &bare_trie, "kaatai", walk::DEFAULT_MAX_BEAM_WORK)
            .analyses
            .is_empty(),
        "baseline: the bare walker must miss the deletion surface form"
    );
    ...
    assert_eq!(
        r.tier,
        compiler::RuleInverseTier::Exact,
        "an env-gated deletion must compile Exact: {:?}",
        r.reasons
    );
    ...
    assert!(
        chain_covers(&g, &chain_trie, std::slice::from_ref(&r2.pinv), "kaatai"),
        "the chain must restore the deleted word-internal segment"
    );
    assert!(
        chain_sigs(&g, &chain_trie, std::slice::from_ref(&r2.pinv), "kaatai")
            .is_subset(&engine_sigs(&g, "kaatai")),
    ...
    assert!(
        !chain_covers(&g, &chain_trie, std::slice::from_ref(&r3.pinv), "kaitai"),
        "a matched non-word must stay unparsed"
    );
}
...
fn chain_recovers_word_internal_epenthesis_with_env_gating() {
    ...
    let morpher = Morpher::new(&g, usize::MAX);
    assert!(
        !morpher.parse_word("atikau").analyses.is_empty(),
        "precondition: the engine parses 'atikau' under the epenthesis rule"
    );
    ...
    let bare_trie = Trie::build(&g, &surface, &build_morpher, 1_000_000, 2, true);
    assert!(
        walk::analyze_word(&g, &bare_trie, "atikau", walk::DEFAULT_MAX_BEAM_WORK)
            .analyses
            .is_empty(),
        "baseline: the bare walker must miss the epenthesized surface form"
    );
    ...
    assert_eq!(
        r.tier,
        compiler::RuleInverseTier::Exact,
        "an env-gated epenthesis must compile Exact: {:?}",
        r.reasons
    );
    ...
    assert!(
        chain_covers(&g, &chain_trie, std::slice::from_ref(&r2.pinv), "atikau"),
        "the chain must strip the epenthesized segment"
    );
    assert!(
        chain_sigs(&g, &chain_trie, std::slice::from_ref(&r2.pinv), "atikau")
            .is_subset(&engine_sigs(&g, "atikau")),
        "soundness: chain candidates must be a subset of the engine's"
    );
    ...
    assert!(
        !chain_covers(&g, &chain_trie, std::slice::from_ref(&r3.pinv), "atukau"),
        "a matched non-word must stay unparsed"
    );
}
...
fn chain_respects_restoration_cap_unconditioned_deletion() {
    ...
    let morpher = Morpher::new(&g, usize::MAX);
    assert!(
        !morpher.parse_word("agau").analyses.is_empty(),
        "precondition: the engine parses 'agau', restoring BOTH deleted d's in one round"
    );
    ...
    let bare_trie = Trie::build(&g, &surface, &build_morpher, 1_000_000, 2, true);
    assert!(
        walk::analyze_word(&g, &bare_trie, "agau", walk::DEFAULT_MAX_BEAM_WORK)
            .analyses
            .is_empty(),
        "baseline: the bare walker must miss the doubly-deleted surface form"
    );
    ...
    assert_eq!(
        cap1.tier,
        compiler::RuleInverseTier::Exact,
        "an unconditioned deletion is compilable (the trie prunes restorations in lockstep): {:?}",
        cap1.reasons
    );
    assert!(
        !chain_covers(&g, &chain_trie, std::slice::from_ref(&cap1.pinv), "agau"),
        "cap respected STRICTLY: 2 restorations under a cap of 1 falls to unparsed"
    );
    ...
    assert!(
        chain_covers(&g, &chain_trie, std::slice::from_ref(&cap2.pinv), "agau"),
        "with cap 2 the chain restores both deleted segments"
    );
    assert!(
        chain_sigs(&g, &chain_trie, std::slice::from_ref(&cap2.pinv), "agau")
            .is_subset(&engine_sigs(&g, "agau")),
        "soundness: chain candidates must be a subset of the engine's"
    );
    ...
    assert!(
        !chain_covers(&g, &chain_trie, std::slice::from_ref(&cap2.pinv), "azau"),
        "a matched non-word must stay unparsed"
    );
}
