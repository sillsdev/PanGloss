...
fn chain_recovers_boundary_conditioned_substitution_after_morpheme_junction() {
    ...
    assert!(!morpher.parse_word("sabata").analyses.is_empty(), "precondition: the toy engine parses 'sabata' as SA-PFX + pata, voiced after the boundary");
    ...
    assert!(walk::analyze_word(&g, &bare_trie, "sabata", walk::DEFAULT_MAX_BEAM_WORK).analyses.is_empty(), "baseline: the bare walker must miss the boundary-voiced surface form");

    let compiled = compiler::compile_default(&g);
    let r = compiled.iter().find(|r| r.name == "voice_after_boundary").expect("rule compiled");
    assert_eq!(r.tier, RuleInverseTier::Exact, "a boundary-only-gated substitution must compile Exact: {:?}", r.reasons);
    ...
    let r2 = compiled2.iter().find(|r| r.name == "voice_after_boundary").expect("rule compiled");
    let non_word = walk::analyze_chain(&g, &chain_trie, std::slice::from_ref(&r2.pinv), "sabala", walk::DEFAULT_MAX_BEAM_WORK, walk::DEFAULT_MAX_BOUNDARY_INSERTIONS);
...
fn chain_respects_insertion_cap_two_boundary_crossings_never_hangs() {
    ...
    let surface = SurfacePhonology::new(&g);
    let compiled = compiler::compile_default(&g);
    let r = compiled.iter().find(|r| r.name == "voice_after_boundary_cap").expect("rule compiled");
    ...
    let under_capped = walk::analyze_chain(&g, &chain_trie, std::slice::from_ref(&r.pinv), "satibata", walk::DEFAULT_MAX_BEAM_WORK, 1);
    let elapsed = start.elapsed();
    assert!(under_capped.analyses.is_empty(), "cap 1 cannot pay for both boundary crossings -- must fall to unparsed");
    assert!(elapsed.as_secs() < 5, "the cap must turn a would-be-hang into a fast 'unparsed', not a slow one");
    ...
    let r3 = compiled3.iter().find(|r| r.name == "voice_after_boundary_cap").expect("rule compiled");
    let non_word = walk::analyze_chain(&g, &chain_trie, std::slice::from_ref(&r3.pinv), "satibala", walk::DEFAULT_MAX_BEAM_WORK, 4);
    assert!(non_word.analyses.is_empty(), "a matched non-word (differs from the lexicon entry after the shared prefix) must stay unparsed");
}
