...
fn chain_recovers_word_internal_rule_two_segment_lhs_far_from_any_boundary() {
    ...
    assert!(
        !morpher.parse_word("katamagai").analyses.is_empty(),
        "precondition: the engine parses 'katamagai'"
    );
    ...
    assert!(
        walk::analyze_word(&g, &bare_trie, "katamagai", walk::DEFAULT_MAX_BEAM_WORK)
            .analyses
            .is_empty(),
        "baseline: the bare walker must miss the substituted surface form"
    );
    ...
    assert!(
        chain_covers(&g, &chain_trie, std::slice::from_ref(&r.pinv), "katamagai"),
        "the chain must recover the word-internal 2-segment substitution"
    );
    assert!(
        chain_sigs(&g, &chain_trie, std::slice::from_ref(&r.pinv), "katamagai")
            .is_subset(&engine_sigs(&g, "katamagai")),
    ...
    assert!(
        !chain_covers(&g, &chain_trie, std::slice::from_ref(&r2.pinv), "katamazai"),
        "a matched non-word (same target substring, different elsewhere) must stay unparsed"
    );
}
...
fn chain_recovers_two_rule_feeding_chain_mid_root() {
    ...
    let morpher = Morpher::new(&g, usize::MAX);
    assert!(
        !morpher.parse_word("aaadbaaai").analyses.is_empty(),
        "precondition: the engine parses 'aaadbaaai' under the feeding cascade"
    );
    ...
    let bare_trie = Trie::build_ex(&g, &surface, &build_morpher, 1_000_000, 2, false, false);
    assert!(
        walk::analyze_word(&g, &bare_trie, "aaadbaaai", walk::DEFAULT_MAX_BEAM_WORK)
            .analyses
            .is_empty(),
        "baseline: the bare walker must miss the doubly-substituted surface form"
    );
    ...
    assert!(
        !chain_covers(
            &g,
            &chain_trie,
            std::slice::from_ref(&rule_b.pinv),
            "aaadbaaai"
        ),
        "rule B's inverse alone cannot see through to rule A's restoration"
    );
    assert!(
        !chain_covers(
            &g,
            &chain_trie,
            std::slice::from_ref(&rule_a.pinv),
            "aaadbaaai"
        ),
        "rule A's inverse alone cannot undo rule B's own substitution"
    );
    ...
    assert!(
        chain_covers(&g, &chain_trie, &chain, "aaadbaaai"),
        "the 2-level chain must recover the feeding cascade"
    );
    assert!(
    ...
    assert!(
        !chain_covers(&g, &chain_trie, &chain2, "aaadbazai"),
        "a matched non-word (same target substring, different elsewhere) must stay unparsed"
    );
}
...
fn chain_recovers_long_distance_harmony_suffix_vowel_agrees_with_first_root_vowel() {
    ...
    assert!(
        !engine.analyses.is_empty(),
        "precondition: 'uptku' = uptk+HARM under root-controlled harmony, got {:?}",
        engine.analyses
    );
    ...
    let bare_trie = Trie::build_ex(&g, &surface, &build_morpher, 1_000_000, 2, false, false);
    assert!(
        walk::analyze_word(&g, &bare_trie, "uptku", walk::DEFAULT_MAX_BEAM_WORK)
            .analyses
            .is_empty(),
        "baseline: the bare walker must miss the harmonized surface form"
    );
    ...
    assert_eq!(
        ...
        "quantified env spans must stay Exact-tier through a real root+suffix walk: {:?}",
        r.reasons
    );
    ...
    assert!(
        ...
        "the chain must recover the harmonized suffix form"
    );
    assert!(
        chain_sigs(&g, &chain_trie, std::slice::from_ref(&r.pinv), "uptku")
            .is_subset(&engine_sigs(&g, "uptku")),
        "soundness: chain candidates must be a subset of the engine's"
    );
...
fn chain_recovers_metathesis_word_internal_consonant_swap_is_a_documented_deferral() {
    ...
    assert_eq!(
        ...
        "metathesis compilation is deferred: the stub is unconditional IdentitySkip"
    );
