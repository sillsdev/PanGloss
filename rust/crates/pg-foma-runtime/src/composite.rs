...
use crate::analyzer::{FomaProposer, ProposalCounts, ProposalDiagnostics};
...
#[doc(hidden)]
pub type ProposeCandidatesErr = (ApplyDimension, usize, usize, ProposalDiagnostics, usize);
...
#[doc(hidden)]
pub struct ProfiledFomaOutcomeWithCandidates {
    pub outcome: FomaOutcome,
    pub diagnostics: FomaWordDiagnostics,
...
#[doc(hidden)]
pub enum ProfiledFomaApplyOutcomeWithCandidates {
...
pub fn propose_union_peel_with_diagnostics(
    g: &Grammar,
...
impl<'g> FomaAnalyzer<'g> {
    ...
    #[doc(hidden)]
    pub fn analyze_word_with_diagnostics_budgeted_with_candidates(
        &mut self,
        word: &str,
        budget: &ApplyBudget,
    ...
    /// (`crate::emission_support::PROBE_STACK_BYTES` — same size `crate::preexpand::build_composites` and
...
pub fn confirm_proposed_words(
    ...
    {
        let mut builder =
            rayon::ThreadPoolBuilder::new().stack_size(crate::emission_support::PROBE_STACK_BYTES);
        if max_threads > 0 {
