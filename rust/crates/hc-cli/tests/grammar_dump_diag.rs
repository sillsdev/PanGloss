...
//! explicitly with:
//! `cargo test -p hc-cli --release --test grammar_dump_diag -- --ignored --nocapture`
...
fn rule_label(g: &Grammar, mid: hc_grammar::model::MRuleId) -> String {
...
fn dump_amharic_grammars() {
    ...
    let (new_grammar, _warnings) = hc_grammar::compile_project(&snapshot).expect("compile");
    let legacy_grammar =
        hc_grammar::load(&std::fs::read_to_string(&oracle_path).expect("read oracle"))
            .expect("load oracle");
