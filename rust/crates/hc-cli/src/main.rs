...
fn run_parse(args: &[String]) -> Result<(), String> {
    ...
    let [grammar_path, word] = positional[..] else {
        return Err("usage: parse <grammar.xml> <word> [--trace[=<file>]] [--trace-format=text|json] [--gloss] [--natural-gloss=eng] [--realize-map=<path>] [--engine=default|foma]".into());
    };
...
fn run_batch(args: &[String]) -> Result<(), String> {
    ...
    let [grammar_path, words_path, out_path] = positional.as_slice() else {
        return Err(
            "usage: batch <grammar.xml> <words.txt> <out.tsv> [--step-cap N] [--word-timeout-ms N] [--memo=on|off] [--threads N] [--start N] [--engine=default|foma]"
                .into(),
        );
    };
    ...
    let xml = fs::read_to_string(grammar_path).map_err(|e| format!("read {grammar_path}: {e}"))?;
    let grammar = hc_grammar::load(&xml).map_err(|e| format!("load {grammar_path}: {e:?}"))?;
    let grammar_load_ms = t_xml.elapsed().as_secs_f64() * 1e3;
