//! P12 chunk 7: rendering a [`TreeTraceSink`] for `hc-rs parse --trace` (design doc §4.3).
//!
...
//! FailureReason)` tuples without caring about whitespace. Both live here (not in `hc-parse`/
//! `hc-rules`) because they need `Grammar` to resolve rule/stratum/template names and a
//! `CharDefTable` to render a `Word`'s shape as text -- exactly the kind of display-only,
//! grammar-aware work `hc-cli` already does for `batch`'s TSV rows (`surface::to_regex_display`
...
use hc_rules::trace::{TraceHandle, TraceNode, TraceSource, TreeTraceSink};
...
fn mrule_name(g: &Grammar, id: hc_grammar::model::MRuleId) -> String {
...
fn template_name(g: &Grammar, id: hc_grammar::model::TemplateId) -> String {
    g.templates
        .get(id.0 as usize)
...
/// (matching `hc-parse::Morpher::surface_of`'s convention) -- correct for every node regardless of
...
fn render_word_shape(g: &Grammar, w: &Word) -> String {
    ...
    hc_parse::surface::to_plain_string(table, &w.shape, false)
}
...
/// `hc-cli` has no `serde` dependency today and adding one purely for this would be more than this
...
mod tests {
    ...
    /// plus a suffix rule appending "+d" (same shape as `hc-parse`'s own `word_timeout_gate.rs`
    /// fixture, reproduced here so `hc-cli` doesn't need a dependency on `hc-parse`'s test helpers).
...
"#;
        hc_grammar::load(XML).unwrap_or_else(|e| panic!("golden_grammar failed to load: {e}"))
    }
    ...
    fn text_render_matches_golden_string() {
        ...
        // (`hc-rules/src/stratum.rs`): `SynthesisStratumRule.Apply` (C#) always explores BOTH the
