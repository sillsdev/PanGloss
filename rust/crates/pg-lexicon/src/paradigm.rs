...
//! every form comes from [`pg_parse::Morpher::synthesize_guessed_stem`], the same
//! `AllomorphId::GUESSED` fabrication `pg_parse::guess::lexical_guess` uses for unparsed words.

use std::collections::{HashMap, HashSet};

use pg_featstruct::FeatureValue;
use pg_grammar::model::{AffixProcessRuleDef, Grammar, MRuleId, MorphRuleDef, MprSet};
...
/// must accept `class_mpr` (`Grammar::mpr_group_ok`, the same group-aware gate `pg-rules`' own
...
fn rule_applies_to_class(
    ...
    class_mpr: MprSet,
    class_pos_bits: Option<pg_featstruct::SymbolBits>,
) -> bool {
