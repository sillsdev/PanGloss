...
//! into a copy of the grammar's original XML text, so the caller (pg-wasm, a later phase) can
//! reload the augmented text through the normal `pg_grammar::load` + `Morpher` construction path.
//! Recognition of user words on future text comes ENTIRELY from that reload; this module never
//! touches the in-memory [`Grammar`].
//!
//! `pg-grammar`'s own loader (`pg_grammar::load`) parses XML into a private, read-only, non-`pub`
...
/// `user:<id>` recognition convention. Child element order is irrelevant to `pg_grammar::load`'s
