...
pub(crate) const DANGLING_REFERENCE: &str = ImportWarningCode::FwdataDanglingReference.wire();
...
pub(crate) const UNEXPECTED_CLASS: &str = ImportWarningCode::FwdataUnexpectedClass.wire();
...
pub(crate) const MISSING_REQUIRED_FIELD: &str =
    ImportWarningCode::FwdataMissingRequiredField.wire();
...
pub(crate) const MISSING_LANG_PROJECT: &str = ImportWarningCode::FwdataMissingLangProject.wire();
...
pub(crate) const ONLY_FIRST_USED: &str = ImportWarningCode::FwdataOnlyFirstUsed.wire();
...
pub(crate) const EMPTY_REPRESENTATION: &str = ImportWarningCode::FwdataEmptyRepresentation.wire();
...
pub(crate) const UNRECOGNIZED_ENUM_VALUE: &str =
    ImportWarningCode::FwdataUnrecognizedEnumValue.wire();
...
pub(crate) const METATHESIS_APPROXIMATION: &str =
    ImportWarningCode::FwdataMetathesisApproximation.wire();
...
pub(crate) const STALE_ADHOC_PROHIBITION: &str =
    ImportWarningCode::FwdataStaleAdhocProhibition.wire();
...
pub(crate) const NO_USABLE_ALLOMORPHS: &str = ImportWarningCode::FwdataNoUsableAllomorphs.wire();
...
pub(crate) const UNSUPPORTED_MORPH_TYPE: &str =
    ImportWarningCode::FwdataUnsupportedMorphType.wire();
/// An allomorph's morph-type guid isn't recognized as any known FieldWorks morph type at all.
pub(crate) const UNKNOWN_MORPH_TYPE_GUID: &str =
    ImportWarningCode::FwdataUnknownMorphTypeGuid.wire();
...
pub(crate) const REFERENCE_NOT_IN_SCOPE: &str = ImportWarningCode::FwdataReferenceNotInScope.wire();
...
pub(crate) const INVALID_PARSER_PARAMETER: &str =
    ImportWarningCode::FwdataInvalidParserParameter.wire();
/// The parser selector is malformed or names a parser this importer cannot represent.
pub(crate) const INVALID_ACTIVE_PARSER: &str = ImportWarningCode::InvalidSourceActiveParser.wire();
/// A guid is shared by more than one `<rt>` record, recognized or not.
pub(crate) const DUPLICATE_GUID: &str = ImportWarningCode::InvalidSourceDuplicateGuid.wire();
/// A tracked (allowed-class) `<rt>` record has no `guid` attribute, or an empty one.
pub(crate) const MISSING_GUID: &str = ImportWarningCode::InvalidSourceMissingGuid.wire();
...
pub(crate) const WRITING_SYSTEM_STORE_UNREADABLE: &str =
    ImportWarningCode::FwdataWritingSystemStoreUnreadable.wire();
