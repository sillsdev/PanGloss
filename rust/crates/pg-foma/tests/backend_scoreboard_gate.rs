...
/// A ratchet, not a target: TSP gained two refusals-turned-exact from per-allomorph zone ownership and the dropped closure refusal (pinned by the outcome table below), TUT one from the same zone fix; a process-morphology refusal and two all-`expect_fail` "misses" (metathesis-comparison-crash) also became exact; earlier moves were a boundary-character fix and the `circumfix-conditioned-halves` fixture (60 -> 61 per strategy) -- `two-table-shared-representation-recall`'s own cross-table emitter fix (`crate::emit::collect_roots`'s bare-root-phonology probe now renders against `surface_table`, not the root's own home-stratum table) turned its one remaining TSP miss into `oracle_exact`, 55/3 -> 56/2. TSP's two REMAINING misses were unrelated to that fix: `morphotactic-attribute-breadth` and `segment-natural-class-table-binding` "g" (a cross-table analysis hc.dll requires that the proposer still never offers, a distinct gap from the one just closed) -- `mpr-gated-exception` is NOT one of the two: it measures `OracleExact` here because the scoreboard compares identity SETS, and its "mentanukam" multiplicity gap (two derivation orders collapsing to one proposal) never shows up as a set-level miss -- see this module's own doc for how each figure was reproduced. `morphotactic-attribute-breadth`'s own miss (TSP+TUT+PC) was a candidate-decode representation bug, not a missing construct, for "kuldede" (a `multipleApplication > 1` rule applied twice): fixed by `crate::tags::collapse_adjacent_repeats`, 56/2 -> 57/1 for TSP and a like TUT gain (see below); PC's own separate miss on this fixture (a RealizationalRule zero-proposal gap) is unaffected.
