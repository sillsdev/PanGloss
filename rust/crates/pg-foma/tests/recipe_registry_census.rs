...
fn some_staged_fixture_separates_more_than_three_registry_transforms() {
    ...
    // the CLI refuses non-baseline candidates outright (`recipe_runtime`'s marker branch). A fixture
    // with markers can still prove plan-level facts; it cannot demonstrate a whole-grammar
