...
//!
//! Every exercise is an ALREADY-COMMITTED conformance fixture and has two ends, exactly as task
//! 7.7's `morphotactics_boundary_cleanup_slice.rs` established:
//!
//! - The TOP end is a claim about the fixture's own GRAMMAR — that the mechanism is really declared
//!   in it (an ordered multi-rule phonological cascade; two strata with different character tables;
//!   a rule carrying `requiredStemName`; a morphological rule whose every subrule's output is pure
...
//! fixture itself ships. A hand-derived expectation would pin this file's author's arithmetic
...
//! - **Ordered morpheme SEQUENCE vs unordered morpheme SET.** The template-order exercise asserts
//!   that `andikishila` and `andikilisha` have EQUAL morpheme sets and DIFFERENT ordered sequences.
//!   That difference is the entire observable content of "template order", and set equality alone
//!   would be blind to it.
//! - **Sequence-difference of size one.** The zero-morphology exercises assert that two readings of
//!   one surface differ by exactly one morpheme key, and name that key.
//!
//! # Two hard constraints this file observes
//!
//! No assertion is a proposal-set ceiling or a truncation: every count here is either read from a
//! committed record or is a lower bound stated as such. And no assertion reads a clock — wall time
//! is never an eligibility or certification input.
//!
...
//! string, that is upstream's committed data, not this file's naming.
//!
//! # Helper duplication is deliberate
//!
//! `committed_words`/`assert_word_parity` mirror 7.7's. Each integration test file is its own crate,
//! so there is no way to share them without editing a third file — which 7.8's split across two
//! agents explicitly forbids. The duplication is the cheaper of the two costs.
