...
//! questions, and one mechanism was being asked to answer both. `crate::backend_optimizer::Score`
...
//!   answerable here. Containment says nothing about cost. `crate::backend_optimizer::Score` and
//!   the certification path remain the whole answer to that, untouched.
//!
//! # Why containment against the oracle is free
//!
//! `crate::backend_runtime::PreparedCorpus` already runs the ground-truth `pg_parse::Morpher` ONCE
...
/// reasoning as `backend_runtime`'s own mismatch-detail sample bound.
...
pub struct CandidateAccuracy {
    ...
    /// `crate::backend_runtime::RuntimeEvaluation::realized_strategy`), and anything attributing a
