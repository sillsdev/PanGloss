...
//! Usage: `pg.ps1 -Mode run -Example lab -- prefilter_census <sena|indonesian|amharic> [word_cap]`; prints a markdown report (category counts, `FailureReason` histogram, time-share ratios). Not part of `cargo test`.
...
fn run(args: Vec<String>) {
    let grammars: Vec<(&str, &str, &str, usize)> = if !args.is_empty() {
        let cap_override = args.get(1).and_then(|s| s.parse().ok());
        match args[0].as_str() {
            "sena" => vec![(
                "Sena",
                "sena-hc.xml",
