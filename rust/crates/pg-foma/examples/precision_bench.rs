...
use pg_foma::precision::{ConstraintCatalog, PrecisionAction, PrecisionConfig};
...
fn propose_and_peel(
    ...
    let peeled = peeler
        ...
        .unwrap_or_else(|e| {
            eprintln!("[precision_bench] reduplication peel refused for {word:?}: {e}");
            Vec::new()
        });
    for c in peeled {
...
fn run_all() {
    ...
    println!(
        "Presets benched: Strip, AllFlags (FullCompile/Auto are emit-identical to Strip this step \
         -- see `pg_foma::precision`'s own doc)."
    );
    println!(
