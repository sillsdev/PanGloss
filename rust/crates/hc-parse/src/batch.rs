...
//!
//! This lives in `hc-parse` (not `hc-cli`) because `hc-parse`'s own module doc says it is the
//! only crate `hc-ffi` and `hc-cli` call — `hc-ffi`'s `hc_parse_batch` FFI entry point (plan
...
//! overflow a default-size thread stack on heavy words (`hc-cli`'s `main()` already works around
...
//! (plan §7, §8 layer 3; enforced by `hc-parse/tests/batch_determinism.rs`).
//!
//! ## Scheduling
...
/// Stack size for each rayon pool worker thread. Matches `hc-cli::main`'s main-thread worker
...
/// `hc-ffi`'s abort-safety test (plan §8 layer 7, M8), which needs a real panic raised *inside a
