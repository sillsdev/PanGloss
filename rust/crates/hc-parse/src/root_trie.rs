...
//!
//! ## Why a direct trie and not `hc-fst`
//! Two independent facts steer the design onto a hand-built trie rather than the frozen `hc-fst`
//! engine:
//!
//! 1. **Multi-accept-id.** The C# trie is one FSA with *many* accepting states, each carrying its
//!    own allomorph id (`RootAllomorphTrie.cs:31,50-52,59`). `hc_fst::CompileInput` compiles one
//!    pattern to one FSA with a *single* `accept_id` (`compile.rs:58`); it cannot express a
//!    1371-path trie whose every leaf carries a distinct root id, and `hc-fst` is a frozen contract
//!    (no edits).
...
//!    [`hc_featstruct::flat_unifiable`] on two empty rows is trivially `true`. A lane-only trie would
...
//!    `char_def` (a [`hc_grammar::chardef::CharDefId`], whose representations are unique per table).
...
//!    build-time [`hc_grammar::chardef::CharDefTable::unifiable_cds`] closure (Design A, P5) is the
...
//! analog), AND [`hc_featstruct::flat_unifiable`] on the phonological lanes.** For a zero-phon-feature
...
//! `char_def` ids are **per-table** ([`hc_grammar::chardef::CharDefId`] is a dense per-table
...
//! - **Stale `char_def` after feature-change.** `hc_shape::ShapeBuilder::modify` deliberately leaves
...
//! `hc-fst` arcs match purely on lane unifiability with no `StrRep`/char-def dimension
...
//! than on the frozen FSA engine; `hc-fst` itself is left untouched.

use hc_featstruct::flat_unifiable;
use hc_grammar::chardef::{CharDefId, CharDefTable};
use hc_grammar::model::{AllomorphId, Grammar, LexEntryId, StratumId, TableId};
use hc_shape::{CdBits, CdSet, EffectiveCdSet, NodeKind, Shape, NO_CHAR_DEF};
...
/// `<PhoneticShape>`, loaded by `hc_grammar::segment::segment_with_patterns`) has
...
/// entries the class unifies with — `hc_grammar::segment`'s `nat_class_cd_set`) plus lane
...
impl RootAllomorphTrie {
    ...
    /// `hc_rules::rewrite::new_seg_node`'s `_ => u32::MAX` fallback). C# unification treats an
