...
//! (`hc-rules/src/morph.rs`'s `compound_match` sites), so the port was pure test-writing (its
...
//! MaxStemCount = 3`, three-root compounding "pʰutdatpip"): `hc_parse::Morpher` hardcodes
//! `max_stem_count: 2` in its `AnalyzerConfig` (see `hc-parse/src/morpher.rs`'s `parse_word`) with no
...
fn root_gloss_set(outcome: &hc_parse::ParseOutcome) -> BTreeSet<String> {
...
/// child `Word` by the time the key was taken. It didn't: `hc-rules/src/morph.rs`'s
...
/// `non_head_app_index -= 1` in `hc-rules/src/stratum.rs`'s `guided_synth`). So the consumed
...
/// file's tests build, but NOT guaranteed by the public API: `hc_parse::Morpher::generate_words`
...
/// `hc_rules::morph::resolve_non_head_roots` performs. Verified against the live C# oracle
...
// `hc-rules/src/morph.rs`), so this port is pure test-writing, unlocked by W5's plan row).
// =================================================================================================
