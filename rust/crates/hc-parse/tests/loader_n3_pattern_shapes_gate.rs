...
//! used only by the root-allomorph loader") -- and that half is done: `hc_grammar::load` no longer
//! silently drops a root allomorph whose `<PhoneticShape>` needs the `[NatClass]`/`([NatClass])`/
//! `[NatClass]*` pattern language (see `hc-grammar/src/segment.rs::segment_with_patterns` and its
//! unit tests, plus `hc-grammar/src/load.rs`'s
...
//! an *actual input word* against such a root allomorph at analysis time. `hc-parse::root_trie`'s
...
//!
//! **FIXED (wave-4, N3 end-to-end):** `TrieEdge` now carries the stored node's `hc_shape::CdSet`
...
/// `hc-grammar`'s own `root_allomorph_shape_falls_back_to_pattern_language_natural_class_reference`
