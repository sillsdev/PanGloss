...
//!
//! Red-on-revert: reverting `hc-rules/src/validity.rs`'s `stem_name_gates_ok` call (the two W5
...
//! Reverting the `apply_blocking` wiring in `hc-rules/src/morph.rs`'s `synthesize`/
