...
//! `hc_parse::Morpher`, the oracle for anything authored/verified against hc-rs (staged fixtures'
...
use hc_conformance_fixtures::{assert_matches_oracle, discover, graduation_guard_violations};
...
/// Every discovered fixture's `words.yaml` replayed against `hc_parse::Morpher`. Pathological
...
fn w91_affix_shapes_covered_by_upstream_fixtures() {
    ...
    let g2 = hc_grammar::load(&truncate.load_grammar_xml()).unwrap();
