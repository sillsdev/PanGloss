...
//! `hc-rules/tests/validity_gate.rs`/`redup_and_free_fluctuation_gate.rs`) are out of scope.
//!
...
//! pre-modification `char_def`/`cd_set` (both the synthesis side, `hc-rules/src/morph.rs::copy_part`,
...
//! both halves were morph-ATTRIBUTION drops in `hc_rules::morph::attribute_morphs` (the C#
...
use hc_featstruct::{FeatureStruct, FeatureStructBuilder, FeatureValue, SymbolBits};
...
/// `hc-rules/tests/validity_gate.rs` uses to drive `hc_rules::morph::synthesize` directly against a
...
fn root_word(g: &hc_grammar::model::Grammar, gloss: &str) -> Word {
...
/// `hc_rules::morph::synthesize`, asserting on the output word's `syn_fs`.
#[test]
fn morphosyntactic_rules() {
    ...
    let out4 = hc_rules::morph::synthesize(&g4, &stem4, &g4.mrules[0]);
...
fn percolation_rules() {
    ...
    for gloss in ["Perc0", "Perc1", "Perc2", "Perc3", "Perc4"] {
        ...
        if !hc_rules::morph::synthesize(&g1, &stem, &g1.mrules[0]).is_empty() {
    ...
    for gloss in ["Perc0", "Perc1", "Perc2", "Perc3", "Perc4"] {
        ...
        if !hc_rules::morph::synthesize(&g2, &stem, &g2.mrules[0]).is_empty() {
...
/// already correct (`hc-rules/tests/morph_gate.rs::simulfix_synthesis_voices_target_segment`, which
/// asserts on `node_lanes` directly, never on the rendered surface string). Root cause, traced
/// directly: a `Modify`-produced `OutNode` (`hc-rules/src/morph.rs::copy_part`, the `char_def: src.
/// shape.char_def(p)` / `cd_set: cd_set_of(src.shape, p)` lines) kept the SOURCE node's own
/// `char_def` (e.g. "p") unchanged, and `Shape::node_cd_set` (`hc-shape/src/lib.rs`) treats any node
/// whose `char_def != NO_CHAR_DEF` as an implicit *singleton* of that original char-def, ignoring the
/// stored `cd_set` entirely. `hc_parse::surface::matching_str_reps` therefore restricted a modified
...
fn simulfix_rules_lane_level_modification_still_works() {
    ...
    let out = hc_rules::morph::synthesize(&g1, &stem, &g1.mrules[0]);
...
/// `hc_rules::morph::attribute_morphs`, not an analysis-cascade gap at all:
