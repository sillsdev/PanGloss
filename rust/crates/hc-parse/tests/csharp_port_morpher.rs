...
//! one line each:**
//! - `TestMatchNodesWithPattern` -- ported (P11 chunk 4) as `hc-parse/src/guess.rs`'s own unit
...
//! rust-conversion.md §7: "intra-word parallelism: cut in phase 1, stays cut"). `hc_parse::Morpher`
...
//! is the exact property `hc-rules/tests/memo_gate.rs` already exercises generically; these 3 tests
...
/// at all -- pins the W6 evaluation gate (`hc-rules/src/validity.rs::allomorph_co_occurrence_ok`)
...
/// `hc-rules/src/validity.rs::morpheme_co_occurrence_ok` at the morpheme granularity.
#[test]
