...
fn fixture_dir(name: &str) -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../conformance/cooccurrence").join(name)
}
...
fn replay(name: &str) -> usize {
    ...
    let xml = std::fs::read_to_string(dir.join("grammar.xml")).unwrap_or_else(|e| panic!("read {name}/grammar.xml: {e}"));
    let grammar = load(&xml).unwrap_or_else(|e| panic!("{name} grammar failed to load: {e}"));
    let morpher = Morpher::new(&grammar, usize::MAX).with_memo(true);

    let text = std::fs::read_to_string(dir.join("expected.tsv")).unwrap_or_else(|e| panic!("read {name}/expected.tsv: {e}"));
    let mut checked = 0;
...
fn morpheme_adjacency_matches_oracle() {
    assert_eq!(replay("morpheme-adjacency"), 16, "expected.tsv should pin all 16 fixture words");
}

#[test]
#[ignore = "conformance/ not yet pulled into PanGloss as a submodule -- see docs/hermitcrab-rust-port-audit.md section 5; will start running again once it lands"]
fn allomorph_basic_matches_oracle() {
    assert_eq!(replay("allomorph-basic"), 4, "expected.tsv should pin all 4 fixture words");
}

#[test]
#[ignore = "conformance/ not yet pulled into PanGloss as a submodule -- see docs/hermitcrab-rust-port-audit.md section 5; will start running again once it lands"]
fn and_semantics_pin_matches_oracle() {
    assert_eq!(replay("and-semantics-pin"), 2, "expected.tsv should pin both fixture words");
}
