...
fn assert_indexing(g: &Grammar, tag: &str) -> RootAllomorphIndex {
    ...
    assert_eq!(index.stratum_count(), g.strata.len(), "{tag}: one trie per stratum");
    ...
    eprintln!("{tag}: {} entries, {total} root allomorphs indexed across {} strata", g.entries.len(), g.strata.len());
    index
}
...
fn assert_real_root_lookups(g: &Grammar, index: &RootAllomorphIndex, tag: &str, limit: usize) -> usize {
    ...
    for s in 0..g.strata.len() {
        ...
        for &le_id in &g.strata[s].entries {
            ...
            let Some(allo) = entry.allomorphs.first() else { continue };
            ...
            for &(a, e) in &got {
                assert!((e.0 as usize) < g.entries.len(), "{tag}: returned entry id out of range");
                assert_eq!(
...
fn sena_root_trie_indexes_and_looks_up() {
    ...
    assert!(g.phon_features.is_empty(), "Sena has zero authored phonological features");
    ...
    assert!(checked >= 10, "sena: expected to verify many real roots, checked {checked}");
}
...
fn indonesian_root_trie_indexes_and_looks_up() {
    ...
    assert!(checked >= 10, "indonesian: expected to verify many real roots, checked {checked}");
}
...
fn sena_negative_lookup_is_end_anchored() {
    ...
    assert!(hit.iter().any(|&(a, _)| a == aid), "positive lookup must hit");
    ...
    b.insert(insert_at, NodeKind::Segment, first_seg_cd, NodeFlags::EMPTY, &[]);
    let longer = b.freeze();
    assert_eq!(
