...
//! `hc_parse::Morpher::parse_word` pipeline over an XML-loaded grammar, matching each C# test's own
...
/// bespoke tiny grammar (matching the style of `hc-rules/tests/validity_gate.rs`) is clearer than
/// growing the shared fixture for one test's two extra POS symbols + one extra entry.
fn build_grammar_with_tv_iv(mrules_xml: &str, templates_xml: &str) -> hc_grammar::model::Grammar {
    let xml = format!(
...
"#
    ...
    hc_grammar::load(&xml).unwrap_or_else(|e| panic!("grammar failed to load: {e}\n---\n{xml}"))
}
