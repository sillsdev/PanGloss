...
fn two_stem_fixture() -> Snapshot {
    ...
    let stem_a = LexEntry {
        ...
        senses: vec![Sense {
            guid: "sense-a".to_string(),
            gloss: vec![ws("en", "dog")],
            definition: Vec::new(),
            msa: Some("msa-a".to_string()),
        }],
        entry_refs: Vec::new(),
    };
    let stem_b = LexEntry {
        ...
        senses: vec![Sense {
            guid: "sense-b".to_string(),
            gloss: vec![ws("en", "cat")],
            definition: Vec::new(),
            msa: Some("msa-b".to_string()),
        }],
        entry_refs: Vec::new(),
    };
...
fn a_representable_cooccurrence_rule_between_two_active_allomorphs_compiles() {
    ...
    assert!(
        has_rule,
        "the resolved rule must actually attach to its primary allomorph"
    );
}
...
fn both_stems_compile_when_nothing_is_unrepresentable() {
    ...
    let out =
        compile_project_with(&snapshot, CompileOptions::default()).expect("fixture must compile");
