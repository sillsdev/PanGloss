...
use hc_fst::{CompileInput, CompileNode, Direction, FstResult, Segment, Transduce, ENTIRE_MATCH};
...
fn accepts_whole(fst: &hc_fst::Fst, s: &[u8]) -> bool {
    if s.is_empty() {
...
fn entire_spans(fst: &hc_fst::Fst, s: &[u8]) -> Vec<(i32, i32)> {
...
fn comparator(deterministic: bool, dir: Direction) -> hc_fst::Fst {
...
fn whole(fst: &hc_fst::Fst, s: &[u8]) -> bool {
    Transduce::new(fst, input(s)).anchored(true, true).accepts()
}
