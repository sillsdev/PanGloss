...
//! [`hc_parse::WordAnalysis`]'s grammar-tier morpheme ordinals against `Grammar::morphemes` to
...
//! (`hc-cli`'s `--gloss` flag today; later milestones' IR/realizer layers build on top of it).
//!
...
/// `u32::MAX` (`hc_grammar::model::MorphemeId::GUESSED`), which is the fabricated root a
...
mod tests {
    ...
    /// pipeline. Pattern follows `hc-parse/src/surface.rs` / `hc-cli/src/main.rs`'s embedded-XML
    /// test fixtures.
...
"#;
    ...
    fn grammar() -> Grammar {
        hc_grammar::load(MINI_GRAMMAR_XML)
