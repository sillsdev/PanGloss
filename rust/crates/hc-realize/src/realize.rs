...
//! this module only defines the shape every realizer produces and the trait a caller (`hc-cli`)
...
/// here as a second `Realizer` impl with no change to any caller — `hc-cli`'s `--natural-gloss`
