...
//! end-to-end through `Morpher` -> `hc_realize::gloss_bundle` -> `hc_realize::to_ir`, loading the
...
//! `hc_realize::gloss_bundle`/`leipzig` directly) to see the actual bundle, THEN writing the
...
use hc_realize::{CaseRole, Concept, GlossIr, Num, Poss, RealizeMap};
...
fn single_analysis_ir(g: &Grammar, map: &RealizeMap, word: &str) -> GlossIr {
    ...
    hc_realize::to_ir(&bundle, map, word)
}
...
fn both_real_sidecar_files_parse_without_error() {
    ...
    assert_eq!(
        amharic.lookup("pl"),
        Some(hc_realize::FeatureAssignment::Num(Num::Pl))
    );
    assert_eq!(
        amharic.lookup("poss.1s"),
        Some(hc_realize::FeatureAssignment::Poss(Poss::P1Sg))
    );
...
fn amharic_pluralized_possessed_noun_maps_both_num_and_poss() {
    ...
    let bundle0 = hc_realize::gloss_bundle(g_ref, &outcome.structured[0]);
    let ir0 = hc_realize::to_ir(&bundle0, &map, "ልጆቹ");
    ...
    let bundle1 = hc_realize::gloss_bundle(g_ref, &outcome.structured[1]);
    let ir1 = hc_realize::to_ir(&bundle1, &map, "ልጆቹ");
...
/// "to-2m" parses with "to" as the *root* token, not an affix, per `hc_realize::to_ir`'s root-
...
fn amharic_sidecar_case_entries_map_as_intended() {
    ...
    assert_eq!(
        map.lookup("at"),
        Some(hc_realize::FeatureAssignment::Case(CaseRole::Loc))
    );
    assert_eq!(
        map.lookup("from"),
        Some(hc_realize::FeatureAssignment::Case(CaseRole::Abl))
    );
    assert_eq!(
        map.lookup("abl"),
        Some(hc_realize::FeatureAssignment::Case(CaseRole::Abl))
    ...
    assert_eq!(
        map.lookup("to"),
        Some(hc_realize::FeatureAssignment::Case(CaseRole::All))
    );
}
...
fn sena_two_morpheme_word_everything_unmapped_goes_to_extras() {
    ...
    // "4-caso": root is "caso" (index 1, per hc_grammar's own root_morpheme_index resolution --
...
// same small synthetic fixture approach n0_gloss_gate.rs and hc-parse/tests/guesser_gate.rs use.
...
fn guessed_root_becomes_guessed_concept_and_never_panics() {
    let g = hc_grammar::load(GUESS_XML)
    ...
    let bundle = hc_realize::gloss_bundle(&g, &outcome.structured[0]);
    let ir = hc_realize::to_ir(&bundle, &RealizeMap::empty(), "gag");
    ...
    for (xml_name, toml_name, words_name) in cases {
        ...
        for word in words.lines().filter(|l| !l.trim().is_empty()).take(100) {
            ...
            for wa in &outcome.structured {
                ...
                let ir_empty = hc_realize::to_ir(&bundle, &RealizeMap::empty(), word);
