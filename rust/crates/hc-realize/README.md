...
Turns a `hc-parse` word analysis into a natural-language gloss phrase -- *"in my houses"*, not
...
(hc-parse, frozen) --N0--> (tokens, root,   --N1--> (Concept, Num,  --N2--> (text, complete,
...
a second `Realizer` impl with **no change to any caller** -- `hc-cli`'s `--natural-gloss` wiring
