...
//! (`hc_grammar::model::LexEntryDef::mpr`, `hc_grammar::model::Grammar::mpr_names`). This module
//! never mutates or rebuilds the grammar; it only reads the already-loaded tables.

use hc_featstruct::FeatureValue;
use hc_grammar::model::{Grammar, LexEntryDef, LexEntryId, MprId, MprSet, SynFeatureKind};
use serde::Serialize;
...
/// symbolic (it always is for a loaded grammar — see `hc_grammar::model::SynFeatureSystem`'s own
/// doc: POS is always feature 0 and always `Symbolic`). Mirrors how `hc-parse::morpher` reads the
...
/// Resolve a [`hc_grammar::model::MorphemeInfo::xml_key`] back to the [`LexEntryId`] that owns it
...
/// The surface (last, per `hc-parse::morpher`'s own convention) stratum's character-definition
...
fn surface_table(grammar: &Grammar) -> Option<&hc_grammar::chardef::CharDefTable> {
...
pub fn validate_shape(grammar: &Grammar, shape: &str) -> Result<(), String> {
    ...
    if hc_grammar::segment::segment(table, shape).is_ok() {
        return Ok(());
    }
...
"#;
    ...
    fn candidate_classes_groups_by_pos_and_mpr() {
        let g = hc_grammar::load(XML).expect("loads");
