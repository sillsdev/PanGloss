//! `hc-lexicon`: "add to dictionary" support for the PanGloss engine (add-to-dictionary design
...
//! without rebuilding the immutable [`hc_grammar::model::Grammar`] the running `Morpher` was built
...
//!   clone in as a sibling — the caller (hc-wasm, a later phase) then reloads the augmented XML
