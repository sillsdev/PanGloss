...
//! hc-lexicon's Sub-project 1 end to end: candidate-class enumeration, disambiguating-form
...
fn candidate_classes_finds_both_noun_classes() {
    let g = hc_grammar::load(TOY_XML).expect("toy fixture loads");
...
fn augment_xml_splices_a_new_entry_that_reloads_and_parses() {
    ...
    let g2 = hc_grammar::load(&augmented_xml).expect("augmented XML reloads cleanly");
    let m2 = hc_parse::Morpher::new(&g2, usize::MAX);
