//! Shared helpers for pg-ffi's integration tests. A `tests/support/mod.rs` (not a top-level
//! `tests/*.rs` file) so Cargo treats it as a module, not its own test binary — the same pattern
//! `pg-grammar`'s tests use for locating the untracked sample corpora (plan §8: "corpora stay
