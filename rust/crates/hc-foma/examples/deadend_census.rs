...
//! SYNTHESIS half of the pipeline (`hc_rules::stratum::synthesize_stratum_traced` and the final
//! `is_word_valid_traced`/`is_match_traced` gates) — confirmed by grep before writing a line of new
//! code: `hc_rules::stratum::StratumAnalyzer`, which runs the ANALYSIS (unapply) cascade every
...
//! (`hc-rules/src/stratum.rs`, `hc-rules/src/morph.rs`, `hc-rules/src/trace.rs`) as ADDITIVE,
...
//!    loop is a sibling under one parent — see `hc-rules/src/stratum.rs`'s own 2026-07-17 doc note).
...
//! 2. `hc-parse::Morpher::lexical_lookup_filtered` (the boundary between the analysis cascade's
...
//! REAL batched `hc_foma::confirm::confirm_batch` (naive per-candidate unbatched sums are
...
//!
//! ## Usage
//!   cargo run -p hc-foma --release --example deadend_census -- <sena|indonesian|amharic> [word_cap]
...
/// convention every trace-emitting call site in `hc-rules` already follows (see `crate::trace`'s
...
/// `hc-rules/src/trace.rs`'s doc on `morphological_rule_not_unapplied` for the sibling case that
...
fn find_frontier(sink: &TreeTraceSink, mode: FrontierMode) -> Option<FrontierHit> {
    ...
    let mut stack: Vec<(hc_rules::trace::TraceHandle, u32)> = vec![(root, 0)];
...
/// too, adding one stack frame per `_traced` wrapper — see `hc-rules/src/morph.rs`'s 2026-07-17
