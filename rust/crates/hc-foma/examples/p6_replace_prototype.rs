...
//! `hc_parse::Morpher` (the SAME oracle/predicate `tests/f2_indonesian_gate.rs` uses), and prints
//! every number the prototype report cites.
//!
//! Run: `cargo run --release -p hc-foma --example p6_replace_prototype`
...
use hc_foma::replace::{compile_and_compose_rules, is_fully_supported_shape, SegAlphabet};
use hc_foma::tags;
...
fn load_indonesian() -> Grammar {
    ...
    hc_grammar::load(&xml).unwrap_or_else(|e| panic!("failed to load indonesian-hc.xml: {e}"))
}
