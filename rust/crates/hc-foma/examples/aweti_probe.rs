//! Throwaway diagnostic: a full `hc_foma::emit::emit(&grammar)` run on Aweti (1022 lex entries, 18
...
//! understood.
//!
//! DO NOT call `hc_foma::emit::emit()` or `crate::preexpand::build_composites` on aweti.json here —
//! that is exactly the OOM this probe exists to avoid re-triggering.
//!
//! Usage: cargo run -p hc-foma --release --example aweti_probe -- [grammar-path]
//!   grammar-path: a `.json` pg-snapshot (loaded via `pg_snapshot` + `hc_grammar::compile_project`,
//!   same path `hc-cli`'s snapshot loader uses) or a legacy `*-hc.xml` grammar (loaded via
//!   `hc_grammar::load`, mirroring `examples/deadend_census.rs::load_grammar`). Defaults to
...
use hc_grammar::model::{AffixAllomorphDef, Grammar, MRuleId, MorphRuleDef, OutputAction, SlotDef};
...
/// `.json` -> pg-snapshot + `hc_grammar::compile_project` (same path `aweti_probe` always used).
/// `*.xml` -> `hc_grammar::load`, mirroring `examples/deadend_census.rs::load_grammar` exactly (the
/// legacy HC XML loader hc-cli and every other example use for Sena/Indonesian/Amharic).
fn load_grammar(path: &Path) -> Grammar {
    let ext = path.extension().and_then(|e| e.to_str()).unwrap_or("");
    if ext.eq_ignore_ascii_case("xml") {
        let xml = std::fs::read_to_string(path)
            .unwrap_or_else(|e| panic!("read {}: {e}", path.display()));
        hc_grammar::load(&xml).unwrap_or_else(|e| panic!("load {}: {e}", path.display()))
    } else {
...
fn main() {
    ...
    let t2 = Instant::now();
    let (should_run, candidate_rule_count, root_count) = hc_foma::emit::composite_scale_hint(g);
    step(&format!(
    ...
    let diag = hc_foma::emit::composite_candidate_rules(g);
    ...
    let n_metathesis = grammar
        .prules
        .iter()
        .filter(|pr| matches!(pr, hc_grammar::model::PhonRuleDef::Metathesis(_)))
        .count();
    let n_epenthesis_rewrite = grammar
        .prules
        .iter()
        .filter(|pr| matches!(pr, hc_grammar::model::PhonRuleDef::Rewrite(r) if r.lhs.nodes.is_empty()))
        .count();
    ...
    // struct_extend (crates/hc-foma/src/emit.rs's second flat-pool builder) has NO template
