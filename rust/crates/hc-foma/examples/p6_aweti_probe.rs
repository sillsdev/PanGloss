...
//! rules compile via `hc_foma::replace`? Scope (per the prototype brief): rule COMPILE only. Does
//! NOT call `hc_foma::emit::emit()` (that is the exact OOM this whole P6 effort routes around —
...
//! composition (`hc_foma::uflexc` is Indonesian-scoped, template-less-morphotactics only; a
//! templated 855-root grammar is out of this prototype's emitter scope entirely, mainline work).
//!
//! Run: `cargo run --release -p hc-foma --example p6_aweti_probe`
...
use hc_foma::replace::{compile_and_compose_rules, compile_rewrite_rule, SegAlphabet};
