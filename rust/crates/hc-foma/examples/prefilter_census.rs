...
//! `hc_rules::validity`'s gate) could have rejected BEFORE the engine ever ran (category a), vs.
...
//!
//! `hc_foma::confirm::confirm_batch`'s whole point is that batching/fusion (root-set grouping,
...
//! using `hc_foma::confirm::confirm_one_traced` (census-only, additive — never on a production
...
//! `hc-parse/src/morpher.rs`'s own doc on why — so this expensive path is scoped to the smaller
...
//! ## Usage
//!
//!   cargo run -p hc-foma --release --example prefilter_census -- <sena|indonesian|amharic> [word_cap]
//!
...
enum Category {
    /// Derives at least one candidate word but is rejected by `hc_rules::validity`'s gate — the
...
/// The validity-gate `FailureReason`s `hc_rules::validity::allomorphs_valid_impl` actually emits
...
/// `hc_rules::trace::TraceSink::failed` is the only method that sets `TraceNode::failure_reason`
...
fn measure_word(
    ...
    for &i in &failing_idx {
        let noop = hc_rules::trace::NoopSink;
