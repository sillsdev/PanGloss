...
//! prototype doesn't attempt -- see `hc-foma/src/gate.rs`'s module doc) with two lexical entries
...
fn main() {
    let g = hc_grammar::load(XML).unwrap_or_else(|e| panic!("failed to load POS fixture: {e}\n{XML}"));
