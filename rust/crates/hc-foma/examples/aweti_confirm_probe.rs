...
//! running `hc-rs batch aweti.json aweti-words.txt out.tsv --engine=foma` after the
...
fn main() {
    // Matches `hc-cli`'s own `main()` (1 GiB dedicated stack) -- the default thread stack
    // overflowed during the compile step alone (recursion in build_composites/emit), so this
    // must run on the same big-stack worker `hc-cli` and `hc_parse::batch::hc_parse_batch` use.
...
fn run() {
    ...
    let (g, _warnings) = hc_grammar::compile_project(&snapshot).expect("compile project");
