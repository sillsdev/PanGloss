...
//! `hc_foma::replace`? Scope (per the prototype brief): COMPILE + tuple-expansion counts +
//! composition sizes only. NO recall gate (the underlying-form lexc emitter in `hc_foma::uflexc`
//! is Indonesian-scoped, template-less-morphotactics only; Amharic uses `<AffixTemplate>` slots
//! this prototype's emitter never attempts) and NO `hc_foma::emit::emit()` call (that is the
...
//!
//! Run: `cargo run --release -p hc-foma --example p6_amharic_probe`
...
use hc_foma::replace::{compile_and_compose_rules, compile_rewrite_rule, is_fully_supported_shape, SegAlphabet};
