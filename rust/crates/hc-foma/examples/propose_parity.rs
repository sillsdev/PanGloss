...
//!   cargo run -p hc-foma --release --example propose_parity > /tmp/before.txt
//!   ... apply changes ...
//!   cargo run -p hc-foma --release --example propose_parity > /tmp/after.txt
//!   diff /tmp/before.txt /tmp/after.txt

use std::path::{Path, PathBuf};

use hc_foma::analyzer::FomaProposer;
use hc_foma::tags::Candidate;
