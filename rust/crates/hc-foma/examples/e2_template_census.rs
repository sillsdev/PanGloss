...
use hc_grammar::model::{Grammar, MorphRuleDef, OutputAction, PartRef};
...
fn main() {
    ...
    let morpher = hc_parse::Morpher::new(&g, usize::MAX).with_word_timeout(Some(std::time::Duration::from_secs(5)));
    let opts = hc_parse::ParseOptions::default();
