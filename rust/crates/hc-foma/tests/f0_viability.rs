...
/// the iterator sugar, matching how the eventual `hc-foma` decoder must enumerate every path
...
fn binary_round_trip_via_file() {
    ...
    let path = std::env::temp_dir().join("hc_foma_f0_roundtrip_file.bin");
...
// foma-rs's `apply_up` output. Not part of the normal `cargo test -p hc-foma` run (no network
// dependency, no binary checked into the repo, per the plan's constraints) -- run manually with
// `cargo test -p hc-foma --test f0_viability -- --ignored --nocapture <name>` and redirect the
