...
//! `AllFlags` preset (`hc_foma::precision`) — THE key property (§0/§6): the knob is
...
//! `hc_foma::composite::FomaAnalyzer::new`/`hc_foma::analyzer::FomaProposer::new` both hardcode
...
//! `confirm_batch`) using only PUBLIC building blocks: `hc_foma::emit::emit_with_precision` to get
//! a lexc source for either preset, the `foma` crate directly to compile+`apply_up` it (mirroring
//! `FomaProposer::propose`'s own logic via `hc_foma::tags::{decode_path, to_candidates}`), and
//! `hc_foma::peel`/`hc_foma::confirm`'s already-`pub` pieces (`ReduplicationPeeler`,
...
//! Indonesian declares zero environment constraints at all (`hc_grammar` XML has no
...
//! coverable instances (`hc_foma::precision`'s own catalog test:
...
use hc_foma::precision::PrecisionConfig;
...
/// Mirrors `hc_foma::analyzer::FomaProposer::propose` exactly (same NFD normalization, same
