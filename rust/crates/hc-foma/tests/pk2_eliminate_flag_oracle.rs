...
//!   parser exposes (and out of hc-foma's FST scope per the design doc's "reduplication stays the
...
fn battery_d_flags_coexist_with_multichar_tags() {
    // `<R:0001>`-shaped multichar symbols are hc-foma's own tag alphabet (see src/tags.rs). This
    ...
    assert!(
        !flag_check("<R:0001>"),
        "a hc-foma tag symbol must never be classified as a flag diacritic"
    );
...
fn battery_e_reduplication_shaped_flags_and_affix_issue60_risk() {
    ...
    // regular-language regex in either compiler's parser, and is explicitly out of hc-foma's FST
...
fn e_flag_type_elimination_not_equivalence_preserving() {
    ...
    // safe arm -- do not assign Eliminate to an E-typed constraint (if hc-foma's emitter ever
