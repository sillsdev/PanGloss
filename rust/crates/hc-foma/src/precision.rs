...
//! `hc_grammar::model::{RootAllomorphDef, AffixAllomorphDef}.environments: Vec<EnvironmentDef>`
...
//! `hc_rules::validity::environments_ok` (see that module) — genuinely diverse: literal segment
...
//!
//! 1. **OR, not AND (`hc_rules::validity::environments_ok`: `envs.iter().any(...)`).** An
...
/// `hc_grammar::model`'s two `environments: Vec<EnvironmentDef>` sites).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
mod tests {
    ...
    fn multi_environment_allomorph_is_or_ambiguous_even_if_individually_simple() {
        // `edge-cases/disjunctive-recheck` is the one fixture the model doc (`hc-rules/src/
