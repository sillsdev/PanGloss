...
//! takes the more direct route available once lexc/rules are built from [`hc_shape::Shape`]
...
//! ([`SegAlphabet::encode_query`], reusing [`hc_grammar::segment::segment_phonemes_only`] — the
...
impl<'t> SegAlphabet<'t> {
    ...
    /// Encode a [`hc_shape::Shape`]'s interior nodes (module doc's "already-segmented" shortcut —
    ...
    pub fn encode_shape(&self, shape: &hc_shape::Shape) -> String {
        shape
            .interior()
    ...
    /// `hc_grammar::segment::segment_phonemes_only` (drop boundaries — a real surface word never
    ...
    pub fn encode_query(&self, word: &str) -> Option<String> {
        let shape = hc_grammar::segment::segment_phonemes_only(self.table, word).ok()?;
        Some(
            shape
                .interior()
                .map(|(_, _, cd, _)| self.token(CharDefId(cd)))
...
fn table_of<'g>(g: &'g Grammar, _sc: &hc_grammar::model::SimpleContext) -> &'g CharDefTable {
    &g.char_tables[0]
}
