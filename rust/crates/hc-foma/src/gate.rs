...
//! entry's own [`hc_grammar::model::LexEntryDef::mpr`] and part of speech are fixed at grammar-load
//! time and never change before the trailing per-stratum phonological-rule cascade runs (the ONLY
//! place `hc_rules::rewrite::subrule_applicable` is ever consulted, `hc-rules/src/stratum.rs`'s
...
//!    applicable to this entry" — by calling `hc_rules::rewrite::subrule_applicable` DIRECTLY (now
...
/// bits belonging to an `Any`-type [`hc_grammar::model::MprGroup`] are excluded from the check
...
fn ungrouped_or_all(g: &Grammar, mpr: hc_grammar::model::MprSet) -> hc_grammar::model::MprSet {
    let mut keep = mpr;
    for group in &g.mpr_groups {
        if group.match_type == MprGroupMatchType::Any {
            keep = hc_grammar::model::MprSet(keep.0 & !group.members.0);
        }
    }
    keep
}
...
/// iff `hc_rules::rewrite::subrule_applicable` (the REAL engine's own predicate — see that
...
pub fn entry_gate_key(
    g: &Grammar,
    entry: &hc_grammar::model::LexEntryDef,
    gated: &[GatedSubrule],
    ...
    gated
        ...
        .map(|gs| {
            ...
            hc_rules::rewrite::subrule_applicable(g, sr, syn_fs, entry.mpr)
        })
        .collect()
}
