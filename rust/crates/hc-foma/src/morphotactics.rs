...
//! automaton over the engine's own morphotactics (`hc-rules/src/stratum.rs`: `synth_apply_mrules`,
...
use hc_featstruct::{is_unifiable, FeatureStruct, FsId, Interner};
use hc_grammar::model::{Grammar, MRuleId, MorphRuleDef, OutputAction, PartRef, SlotDef};
...
struct TemplateInfo {
    ...
    /// template has no stratum field of its own in `hc_grammar::model`).
    owning_stratum: u8,
...
mod tests {
    ...
    /// NOR any slot -- the "rule with no sites" case. Mirrors `crates/hc-foma/src/emit.rs`'s own
...
<HermitCrabInput>
  <Language>
    ...
    /// Mirrors `crates/hc-foma/src/emit.rs`'s own `structural_and_pattern_tests::entry_id_of`
    /// (module doc) -- finds a rule by the XML `id` its owning morpheme's `xml_key` recorded
    /// (`hc-grammar/src/load.rs`: `xml_key: mr.attr("id")...`, the loader's convention for EVERY
