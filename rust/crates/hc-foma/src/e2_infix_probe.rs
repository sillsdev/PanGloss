...
//! composite entries — via the plain, non-recursive `hc_rules::morph::synthesize`, not
...
//! For each (root allomorph × Infix rule) pair: seed a `hc_rules::word::Word` EXACTLY the way
//! `crate::preexpand::process_root_work` does (feature-bearing re-segmentation via
//! `hc_rules::shape_feat::segment_with_features`, the entry's own REAL `syn_fs`/`mpr` — never
//! `FeatureStruct::EMPTY`, the documented `crate::emit::probe_surface` blind spot this module
//! deliberately avoids by construction), call `hc_rules::morph::synthesize` (the plain sibling of
...
use hc_featstruct::is_unifiable;
...
fn required_category_u(g: &Grammar, mid: MRuleId) -> hc_featstruct::FsId {
    match &g.mrules[mid.0 as usize] {
        ...
        MorphRuleDef::Compounding(_) => hc_featstruct::FsId(0),
    }
}
...
fn build_slot_chain_u(
    ...
    slots: &[&SlotDef],
    zone_role: Role,
    template_category: hc_featstruct::FsId,
    width: usize,
    exit: &str,
    uncovered: &mut Vec<String>,
    lines: &mut usize,
...
/// Replays `hc_parse::Morpher::allomorphs_in_morph_order`'s own algorithm — same as
...
/// cleared by a feature-changing rule" (`char_def == hc_shape::NO_CHAR_DEF`), or a node whose OWN
...
/// spliced via `hc_rules::morph::synthesize`. The ORIGINAL, ungated `SegAlphabet::encode_shape`
...
fn encode_shape_variants(alphabet: &SegAlphabet<'_>, shape: &hc_shape::Shape) -> Vec<String> {
    ...
    for (i, _, char_def, _) in shape.interior() {
        ...
        let fast_path = if char_def != hc_shape::NO_CHAR_DEF {
            let cd = table.get(hc_grammar::chardef::CharDefId(char_def));
            if hc_featstruct::flat_unifiable(lanes, cd.feature_lanes()) {
                Some(alphabet.token(hc_grammar::chardef::CharDefId(char_def)))
            } else {
                None
            }
        } else {
...
fn splice_extend_u(
    ...
    for &mid in rules {
        ...
        *pairs_probed += 1;
        ...
        let results = hc_rules::morph::synthesize(g, base_word, rule);
        if results.len() > 1 {
            *ambiguous_pairs += 1;
        }
        for w in &results {
    ...
    for sd in &g.strata {
        for &entry_id in &sd.entries {
            ...
            for allo in &entry.allomorphs {
                ...
                let Ok(shape) = hc_rules::shape_feat::segment_with_features(g, root_table, &allo.shape.text) else {
                    continue;
                };
...
pub fn emit_underlying_amharic_probe(g: &Grammar, alphabet: &SegAlphabet<'_>) -> UProbeResult {
    ...
    let mut group_keys: Vec<hc_featstruct::FsId> = Vec::new();
