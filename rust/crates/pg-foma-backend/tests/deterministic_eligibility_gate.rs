...
fn plans(grammar: &pg_grammar::model::Grammar) -> Vec<pg_foma::enumerate::LoweredCandidate> {
    ...
    pg_foma_backend::backend_registry::Registry::seeded()
        .materialize_distinct(&pg_foma_backend::backend_registry::MaterializerContext {
            grammar,
            baseline: &baseline,
        })
...
fn exclusions_are_candidate_independent() {
    ...
    let evaluations = pg_foma_backend::backend_runtime::evaluate_plans_with_cache(
