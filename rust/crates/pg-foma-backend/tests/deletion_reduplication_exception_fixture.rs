...
use pg_foma_backend::backend_runtime::{evaluate_plans, RuntimeBudget};
...
fn every_distinct_plan_fully_confirms_or_refuses_markers_explicitly() {
        ...
        .expect("the oracle liveness net / memory ceiling must not trip on this fixture")
    {
        match result.certification {
            pg_foma_backend::backend_optimizer::Certification::FullHcConfirmed { .. } => {}
            pg_foma_backend::backend_optimizer::Certification::Unsupported { ref reason } => {
                assert!(
