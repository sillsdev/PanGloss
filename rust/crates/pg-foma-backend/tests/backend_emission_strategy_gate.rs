...
use pg_foma_backend::backend::{Backend, LexcMainline};
...
/// `FomaProposer::EMISSION_STRATEGY` must name the strategy selected by the mainline backend.
#[test]
fn the_named_backend_is_the_one_the_proposer_builds() {
    assert_eq!(
        FomaProposer::EMISSION_STRATEGY,
        LexcMainline.strategy(),
        "the backend's named strategy and the proposer's constructor must agree"
    );
    assert!(
        FomaProposer::EMISSION_STRATEGY.is_whole_grammar(),
        "the proposer compiles the whole grammar, not the controllable subtree"
    );
}
...
fn a_boundary_free_grammar_compiles_through_the_token_cascade_path() {
    ...
    let output = pg_foma_backend::templated_compile::compile_templated_morphotactics(&grammar)
