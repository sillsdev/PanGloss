...
use pg_foma::capability::{characterize_call_count, reset_characterize_call_count};
use pg_foma::characterization::characterization_findings;
use pg_foma::enumerate::enumerate_candidates;
use pg_foma::grammar_semantics::GrammarSemantics;
use pg_foma::junctions::PhonologyProbe;
use pg_foma_backend::backend_registry::Applicability;
...
/// `select_backends` characterizes the grammar once, not once per `EmissionStrategy`.
#[test]
fn select_backends_characterizes_the_grammar_once_not_once_per_strategy() {
    let g = load(GATED_TWO_GROUP_XML);
    let ro = prules_in_order(&g);
    let phon = PhonologyProbe::new(&g);

    // Not select_backends's own input; pins this fixture's gate-key branching precondition.
    let candidates = enumerate_candidates(&g, &ro, phon.as_ref());
    assert_eq!(
        candidates.len(),
        2,
        "this fixture must yield 2 candidates, or its gate-branching precondition is vacuous"
    );

    reset_characterize_call_count();
    let selection = pg_foma_backend::backend_selection::select_backends_for_grammar(&g);
    let select_calls = characterize_call_count();

    assert_eq!(
        selection.reports().len(),
        pg_foma::strategy_coverage::ALL_STRATEGIES.len(),
        "select_backends must report every strategy"
    );
    assert!(
        select_calls > 0,
