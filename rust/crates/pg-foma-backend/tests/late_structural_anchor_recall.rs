...
fn fixture_xml() -> String {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../pg-foma/tests/fixtures/pangloss/fst-completeness/late-structural-anchor-five-rule-chain/grammar.xml");
...
fn assert_product_analysis_matches_oracle(grammar: &Grammar, surface: &str) {
    ...
    let (proposer, _) = pg_foma::analyzer::compile_proposer_unproven_with_profile(grammar);
...
fn complex_allomorph_fixture_xml() -> String {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../pg-foma/tests/fixtures/pangloss/fst-completeness/complex-inserted-redup-later-allomorph/grammar.xml");
    std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("read {}: {error}", path.display()))
}
...
fn later_complex_allomorph_is_recalled_without_hiding_the_first() {
    ...
    let mut analyzer =
        pg_foma::composite::compile_analyzer(&grammar).expect("PanGloss analyzer must compile");
