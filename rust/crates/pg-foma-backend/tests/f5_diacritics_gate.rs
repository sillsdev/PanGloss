...
fn b_bare_propose_finds_every_diacritic_word() {
    let g = load_dia();
    let mut proposer = pg_foma::analyzer::compile_proposer(&g).expect("dia-hc.xml compiles");
    for &word in WORDS {
...
fn c_foma_analyzer_matches_engine_exactly() {
    let g = load_dia();
    let mut analyzer = pg_foma::composite::compile_analyzer(&g).expect("dia-hc.xml compiles");
...
fn e_boundary_mark_foma_analyzer_matches_engine_exactly() {
    let g = load_boundary_mark();
    let mut analyzer =
        pg_foma::composite::compile_analyzer(&g).expect("boundary-mark-hc.xml compiles");
    let morpher = Morpher::new(&g, usize::MAX);
    let opts = ParseOptions::default();
