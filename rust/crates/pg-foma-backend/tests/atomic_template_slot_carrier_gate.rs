use pg_foma_backend::templated_compile::{compile_templated_morphotactics, TemplatedCompileError};

fn mixed_slot_xml() -> String {
    std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join(
            "../pg-foma/tests/fixtures/pangloss/fst-completeness/atomic-template-slot-carrier/grammar.xml",
        ),
    )
