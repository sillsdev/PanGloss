...
fn non_document_order_analysis_is_proposed_and_confirmed() {
    let g = load(&fixture_xml("unordered"));
    let mut analyzer = pg_foma::composite::compile_analyzer(&g).expect(
...
fn linear_variant_of_the_same_grammar_does_not_confirm_the_reverse_order() {
    ...
    let mut analyzer =
        pg_foma::composite::compile_analyzer(&g).expect("Linear fixture must compile too");
...
fn same_rule_reapplication_is_over_proposed_and_confirm_pruned() {
    let g = load(&fixture_xml("unordered"));
    let mut analyzer = pg_foma::composite::compile_analyzer(&g).expect("fixture must compile");
    let morpher = Morpher::new(&g, usize::MAX);
