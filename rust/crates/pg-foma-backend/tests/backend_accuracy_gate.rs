...
use pg_foma_backend::backend_accuracy::{candidate_admission_key, AccuracyVerdict};
use pg_foma_backend::backend_registry::{MaterializerContext, Registry};
...
fn a_removed_proposal_is_reported_as_the_exact_lost_analysis() {
    ...
    let counters =
        pg_foma_backend::backend_accuracy::check_occurrence(&word, 0, &oracle, &[], &mut misses);
    assert_eq!(
    ...
    let verdict = pg_foma_backend::backend_accuracy::verdict_from(&counters, misses);
