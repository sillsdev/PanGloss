...
/// Every crate-qualified `tests/<name>.rs` (or `src/<name>.rs`) path token appearing in `citation`, found by scanning for the `.rs` suffix and walking backwards over the path token rather than imposing a grammar on the free prose.
fn cited_paths(citation: &str) -> BTreeSet<String> {
    ...
    while let Some(rel) = citation[i..].find(".rs") {
        ...
        if token.starts_with("pg-foma/tests/")
            || token.starts_with("pg-foma/src/")
            || token.starts_with("pg-foma-backend/tests/")
            || token.starts_with("pg-foma-backend/src/")
...
/// Every test-function identifier referenced by `citation`, in either of the two forms `containment_evidence_for` uses: `<crate>/tests/<file>.rs::<ident>`, or a `(+ <ident>, ...)` continuation listing sibling tests, filtered to snake_case so prose words aren't mistaken for identifiers.
...
/// Resolve the citation's explicit crate prefix relative to this crate's root.
fn cited_file_path(root: &Path, rel: &str) -> Option<PathBuf> {
    if let Some(path) = rel.strip_prefix("pg-foma-backend/") {
        Some(root.join(path))
    } else if let Some(path) = rel.strip_prefix("pg-foma/") {
        Some(root.parent()?.join("pg-foma").join(path))
    } else if rel.starts_with("tests/") || rel.starts_with("src/") {
        Some(root.join(rel))
    } else {
        None
    }
...
/// Read source from both compiler-core and backend crates; ledger evidence can live in either.
fn cited_source_haystack(root: &Path) -> String {
    let mut all = concat_rs_sources(&root.join("tests"));
    all.push_str(&concat_rs_sources(&root.join("src")));
    let pg_foma_root = root
        .parent()
        .expect("pg-foma-backend must be a sibling of pg-foma")
        .join("pg-foma");
    all.push_str(&concat_rs_sources(&pg_foma_root.join("tests")));
    all.push_str(&concat_rs_sources(&pg_foma_root.join("src")));
    all
}
...
fn every_ledger_citation_names_a_file_that_exists() {
    ...
    for row in &ledger.rows {
        ...
        for rel in cited_paths(&ev.citation) {
            checked += 1;
            if !cited_file_path(&root, &rel).is_some_and(|path| path.is_file()) {
...
fn every_ledger_citation_names_a_test_fn_that_exists() {
    let ledger = build_ledger(&default_registry(), &std::collections::HashSet::new());
    let root = crate_root();
    let haystack = cited_source_haystack(&root);
    assert!(
        !haystack.is_empty(),
