...
use pg_foma::emit;
use pg_foma_backend::backend_selection::best_case_for_grammar;
...
fn depth_budgeted_compound_loop_now_proposes_the_bounded_recursive_shape() {
    let g = load();
    let mut proposer = pg_foma::analyzer::compile_proposer(&g)
        .expect("fixture must compile (a single, simple CompoundingRule)");
...
fn depth_budgeted_compound_loop_contains_the_raised_cap_oracle_analysis() {
    let g = load();
    let mut proposer = pg_foma::analyzer::compile_proposer(&g).expect("fixture must compile");
    let candidates = proposer.propose("tevimaflisra");
    assert!(
        !candidates.is_empty(),
...
fn depth_bound_is_respected_a_k_plus_one_stem_word_is_never_proposed() {
    ...
    let mut proposer =
        pg_foma::analyzer::compile_proposer(&g).expect("small bound fixture must compile");
