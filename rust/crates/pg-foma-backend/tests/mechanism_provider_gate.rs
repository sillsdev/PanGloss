...
use pg_foma_backend::mechanism_provider::derive_mechanism_graph;
...
fn kinds(graph: &pg_foma_backend::backend_mechanism::MechanismGraph) -> Vec<MechanismKind> {
...
fn an_inert_reduplication_hint_creates_no_copy_process_mechanism() {
    ...
    for strategy in [
        ...
        assert_eq!(
            pg_foma_backend::backend_mechanism::MechanismBinding::derive(copy, strategy)
                .disposition(),
...
fn every_requirement_and_source_traces_to_an_observation() {
    ...
    for node in &graph.nodes {
        ...
        for requirement in &node.construct_requirements {
            ...
            assert_eq!(
                pg_foma_backend::backend_mechanism::mechanism_kind_for(*requirement),
                node.kind(),
