...
//!
//! A [`CompletedBackendBuildEvidence`] is deliberately not constructible by callers.  It contains the
...
pub fn compile_completed_backend(
    ...
    let measured = compile_completed_backend_for_measurement(
        grammar,
        &selection,
        requested_strategy,
        request,
...
fn compile_completed_backend_for_measurement(
    ...
    crate::backend::backend_for(requested_strategy)
...
pub(crate) fn compile_templated_underlying_for_measurement(
    ...
    let report = output.proposer.report.as_ref().ok_or_else(|| {
        CompletedBuildError::IncompleteEvidence("templated compiler emitted no report".to_string())
    })?;
    validate_emit_report(report)?;
