...
//! stored subtree instead of re-searching (`hc_rules::stratum`'s memoized cascade).
//!
...
//!   all-`u32` key, but the current `hc_rules::Word` owns its `Shape`/`FeatureStruct` (per-parse
...
//!   not depend on `hc-rules` (which depends on this crate — a cycle otherwise). `hc-rules`
...
use hc_grammar::model::{MRuleId, StratumId};
use hc_shape::Shape;
...
/// process-stable hasher as every other accumulator in the `hc-parse`/`hc-rules` pipeline instead of
...
impl AnalysisStateKey {
    /// Build a key from a word's already-extracted components (`hc-rules` supplies these from a
