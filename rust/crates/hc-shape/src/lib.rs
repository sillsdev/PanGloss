...
//! (raw `u64`, so [`node_lanes`](Shape::node_lanes) feeds `hc_featstruct::flat_unifiable` with no
...
/// dimension, the port's analog of C#'s `StrRep` disjunction). **Not** `hc_featstruct::SymbolBits`:
...
impl CdBits {
    ...
    /// consumer (`hc_rules::morph::segs_of`), which only engages on tables of ≤ 64 char-defs.
...
impl Shape {
    ...
    /// Node `i`'s feature lanes (`W` `u64`s), ready to pass to `hc_featstruct::flat_unifiable`.
...
impl ShapeBuilder {
    ...
    /// hc-shape is pure storage, so the **caller** computes the post-union lanes (reading
    /// [`Shape::node_lanes`], unifying with `hc_featstruct` ops + the grammar's per-feature masks)
...
/// the generic [`Interner`] from `hc-featstruct` but hands back a distinct [`ShapeId`] type.
...
impl ShapeInterner {
    ...
    pub fn get(&self, id: ShapeId) -> &Shape {
        self.inner.get(hc_featstruct::FsId(id.0))
    }
