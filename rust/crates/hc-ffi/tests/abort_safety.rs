...
//! "does `catch_unwind` exist somewhere". The injection point is `hc_parse::batch`'s
//! `TEST_PANIC_WORD` sentinel (see `hc-parse/src/batch.rs`), compiled in ONLY when hc-parse is
//! built with the `test-panic-hook` Cargo feature — which hc-ffi enables *exclusively* via its
...
//! - `cargo build` / `cargo build --release -p hc-ffi` (the shipped `hermit_crab.dll`) does
...
//!   convention.
//!
//! If `hc_parse::batch::TEST_PANIC_WORD` fails to resolve, that itself is a loud signal the
