...
mod tests {
    ...
    /// crate has no dependency on `hc-parse`'s dev-only test helpers).
...
"#;
    ...
    fn generates_root_plus_suffix() {
        // ordinals: `hc_grammar::load`'s `load_stratum` visits `MorphologicalRuleDefinitions`
        // BEFORE `LexicalEntries` (document order within one stratum, `hc-grammar/src/load.rs`'s
