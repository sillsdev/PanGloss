...
//! for `hc_parse_batch`, the call into `hc_parse::hc_parse_batch`'s rayon scoped pool, so a panic
...
/// `hc_parse::hc_parse_batch`, which treats `0` as rayon's default). Internally parallel — do not
...
pub unsafe extern "C" fn hc_parse_batch(
    ...
    let result = std::panic::catch_unwind(|| -> Result<Vec<u8>, i32> {
        ...
        let outcomes = hc_parse::hc_parse_batch(&gh.morpher, &rust_words, max_threads as usize);
