...
| 6 | `SimultaneousRewrite` | `ConfigPredicate` | `simultaneous.subrule-overlap` (`SimultaneousSubruleOverlapPredicate`) | Closed: pairwise-non-overlapping subrules (via `lower.rs`'s real span intersection, `subrules_pairwise_verdict`) → **`Admit`** (already the strongest rung any construct has reached). Open: self-opaquing OR a genuine lowered-span intersection (real overlap) OR an unsupported span → `Refuse`. | ADR 0001's own cited example, verbatim: "the oracle itself is unverified for a configuration (e.g. simultaneous-subrule overlap, never pinned against `hc.dll`)... unsupported by definition." The blocking question is not construction quality — `lower.rs`'s span intersection is already the real evidence, per this task's own framing — it is that HermitCrab's actual behavior for genuinely overlapping subrule environments has never been independently verified. | ~~A C#-oracle-authored fixture with two subrules whose environments provably overlap, once the harness exists.~~ **DONE 2026-07-26** — `conformance-staging/edge-cases/simultaneous-subrule-genuine-overlap`, ground truth from `hc.dll` itself (the first such fixture in the repo). | **ORACLE GAP DISCHARGED → ordinary open construct row.** The harness turned out to largely exist already: `hc.dll` builds, and `machine/conformance/adapters/hc-dotnet-wrapper.sh` already bridges it to the protocol's 3-arg contract, so "zero code exists" was stale. `hc.dll` loaded and compiled a genuinely-overlapping grammar without error, and its `(word, signature)` output is **byte-identical to `pg_parse::Morpher`'s across all 9 words**. Crucially the agreement is *discriminating*, not a shared silence: `be` analyzes and `de` does not, which pins the resolution order (first-declared subrule wins) — had subrule 2 won, those two results would be swapped. So ADR 0001's stated blocker ("the oracle itself is unverified") is discharged for this configuration. **The proposer-side refusal is unaffected** and remains correct: `simultaneous.subrule-overlap` still `Refuse`s on the compiled path with a real span-intersection witness, which is a *construction* question, not an oracle-trust one. Scope of the verification is stated in the fixture's STAGING.md and does not generalize past two subrules sharing one focus with one shared class member. |
...
| 8 | `Metathesis` | `ConfigPredicate` | `metathesis.faithful-swap-construction` | Closed: `Dir::LeftToRight` in-scope shapes → `ConfirmOnly`, `Covered`. Open: `Dir::RightToLeft` metathesis has **zero construction attempted at all** (`compile_metathesis_rule`'s own doc: "out of scope for this change's swap construction") — this is not an unproven existing construction, it is a not-yet-built compiler feature. | **RESOLVED 2026-07-25 — not from scratch.** Reuse `replace.rs`'s existing mirror-and-reverse RTL construction: build the mirror pattern (`reversed_slots`, `:394`, plus a mirrored remap of `left_switch`/`right_switch`), run the existing swap construction on it, `fsm_reverse` (`:245`), and `fsm_union` with the plain net (`:457`) — the same four moves `compile_rtl_branch_net` already makes. C# pairs `IterativePhonologicalPatternRule` with `Direction` identically for metathesis and rewrite (`SynthesisMetathesisRule.cs`), so the faithfulness argument transfers unchanged. | A new RTL-metathesis fixture: `right-to-left-metathesis-reversal` (pattern basis authored, `docs/conformance/representative-typology-basis.md`). | **PROVABLE** (was NEEDS-DECISION; resolved in `docs/conformance/needs-decision-resolutions.md` — build it, lands `ConfirmOnly` as a superset exactly like `RightToLeftRewrite`; the "from scratch" premise was what failed) |
...
| 10 | `CircumfixOutputAction` | `ConfigPredicate` | `circumfix-output-action.faithful-structural-composite` | Closed: `structural_composite_attempted == true` (reaches `build_structural_composites`) → `ConfirmOnly`, `Covered`. Open: `structural_composite_attempted == false` → `Refuse`. | **CENSUS DONE 2026-07-25** — `docs/conformance/circumfix-structural-composite-census.md`. The *mechanism* needs no extension: `build_structural_composites` is allomorph-complete because `struct_extend` delegates to `pg_rules::morph::synthesize` (`emit.rs:2272`), so nothing is overclaimed inside it. Every gap is in candidate **selection**, and all three fail in the over-refusing (honest, fail-closed) direction: **C1** `rule_role` reads allomorph 0 only (`emit.rs:555-560`) so a circumfix allomorph at index ≥ 1 is never selected — order-of-declaration-dependent, highest priority, mechanical fix; **C3** `classify_affix`'s interior-action test (`:434-440`) returns `Infix` before the leading-AND-trailing test (`:441-453`) can return `CircumfixPrefix`; **C2** the same preemption by the reduplication test (`:408-414`), which is NOT independent — it is a joint decision with row 11's carve-out. C4 (`Role::Process` with no `Copy`) and C5 (`has_unemittable_action`) were checked and are correctly not gaps. | One fixture each for C1 and C3 (a rule whose non-first allomorph is circumfix-shaped; a simultaneously-circumfixing-and-infixing RHS). C2 waits on row 11. | **PROVABLE**, gap shapes now named (tasks.md 4.3a/4.3b/4.3c) |
...
| 13 | `MultiTable` | `ConfigPredicate` | `multi-table.faithful-table-threading` | Closed: `representations_pairwise_disjoint == true` → `ConfirmOnly`, `Covered` (two fixtures, incl. the stronger two-table-disagreement case). Open: tables share a representation → `Refuse` ("the residual case this change's threading fix cannot make faithful"). | **DESIGN DONE 2026-07-25** — `docs/conformance/multitable-shared-representation-design.md`. The disjoint-token-range encoding this row originally proposed is **WITHDRAWN as the wrong fix**: `token()` is a table-blind per-table index (`replace.rs:275-277`), so two tables sharing a spelling at *different* indices make a rule fail to fire on the other table's material — a **false negative**, unrecoverable under propose-and-confirm, and the opposite direction from the false-positive risk the predicate's own doc describes. Range separation would entrench it by construction. Recommended instead: **cross-table representation aliasing** — render an atom as a union over every table's token for the same NFD-normalized spelling, applied at `render_slots` only, leaving `owning_table`'s per-table resolution untouched. Recall-safe by construction (only ever adds alternatives to an atom), requires no `model.rs` change (R1's frozen-model audit stays closed, unlike a global-`CharDefId` scheme), and avoids the PUA-capacity question entirely. Corroborating: `bridge.rs:260-300` shows the oracle matches natural classes by feature lanes, not char-def identity. | A new fixture with two tables that legitimately share a spelling AND a second-table rule that must fire on first-table material — one that LOSES an analysis today; the recall-side counterpart to `bistratal-overlapping-segment-representation`'s refusal-side coverage. | **PROVABLE**, design settled (tasks.md 4.4a done, 4.4b is the build). Scope is now smaller than this row assumed: the change is confined to `pg-foma` — a representation multimap, a `TableId` threaded into `SegAlphabet`, and render-time alias expansion. |
| 14 | `QuantifierPattern` | `ConfigPredicate` | `quantifier.bounded-expansion` | Closed: every quantifier bounded and the rule otherwise compiles → `ConfirmOnly`, `Covered`. Open (two distinct sub-splits): (a) a genuinely unbounded quantifier (`max == -1`, the Kleene sentinel) → `Refuse`; (b) all-bounded but blocked by some *other* unsupported construct in the same rule → `Refuse` (inherits that other construct's own row, not a `QuantifierPattern`-specific gap). | Sub-split (a) is genuinely unclear from available evidence: the predicate's own doc frames the refusal as "a finite cutoff must never masquerade as unbounded Kleene semantics," which reads as a *semantic honesty* concern (do not silently truncate something the grammar declared unbounded) rather than a proof that foma's native (regular-language) Kleene star cannot express a truly unbounded repetition faithfully. No document read for this plan says which is actually true. | A new fixture: `unbounded-iterative-quantifier-expansion` (pattern basis authored, `docs/conformance/representative-typology-basis.md`). | **PROVABLE** on sub-split (a) (was NEEDS-DECISION; resolved in `docs/conformance/needs-decision-resolutions.md` — **simply unattempted**). The backend has a native exact unbounded construction (`foma` `regex.rs:258-268` `^>N` = `concat_n(N)·kleene_plus`, `:302` `*` = `fsm_kleene_star`) — finite net, infinite language, no cutoff anywhere; `lower.rs:303-309`'s refusal is a scope line inherited from `compile-bounded-fst-quantifiers`, and the ADR 0001 honesty concern it cites argues against *clamping*, not against emitting a genuine star. Priority is higher than the row assumed: `max` **defaults to `-1`** in the loader (`XmlLanguageLoader.cs:1408-1409`, DTD `:560-568` `#IMPLIED`), so this is a hole on the most common authored shape. Sub-split (b) is still not this row's own gap. |
...
> **RESOLVED 2026-07-25** — `docs/conformance/needs-decision-resolutions.md` §"§D6's held question".
> Neither NEEDS-DECISION row needs a C#-oracle precondition (row 14's rarity premise does not hold —
> unbounded is the DTD *default*; row 8 reuses an already-accepted construction). `SimultaneousRewrite`'s
> overlapping-subrule configuration — the one case ADR 0001 itself names — stays oracle-blocked,
> unchanged.
...
4. ~~Escalate the NEEDS-DECISION rows (`Metathesis` RTL, `QuantifierPattern` unbounded) plus D6's oracle-
   ...
   **DONE 2026-07-25** — decision record: `docs/conformance/needs-decision-resolutions.md`. **Both rows
   resolved PROVABLE — build, no carve-out.** Row 14 (`QuantifierPattern` unbounded): the FST backend has
   a native exact unbounded construction (`foma` `regex.rs:258-268`/`302`), `lower.rs:303-309`'s refusal
   is a scope line from `compile-bounded-fst-quantifiers` rather than a feasibility finding, and
   `max = -1` is the DTD/loader **default** (`XmlLanguageLoader.cs:1408-1409`) so this is a hole on the
   most common authored shape. Row 8 (`Metathesis` RTL): not from scratch — `replace.rs`'s
   `reversed_slots`/`fsm_reverse`/`fsm_union` mirror-and-reverse construction (module doc lines 71-107,
   `:394`, `:457`) transfers directly, and C# uses the same `IterativePhonologicalPatternRule` +
   `Direction` pairing for both metathesis and rewrite. Both land `ConfirmOnly` (superset under
   propose-and-confirm), so both fold into step 3's Stage-2 kit sequence.
